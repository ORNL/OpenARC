...
DEFAULT_TARGET_SYSTEM = BRISBANE
else
...
install: 
	...
	if [ -f ./openarcrt/libopenaccrt_iris.a ]; then cp -f ./openarcrt/libopenaccrt_iris.a $(INSTALL_PREFIX)/lib/; fi
	if [ -f ./openarcrt/libopenaccrtomp_iris.a ]; then cp -f ./openarcrt/libopenaccrtomp_iris.a $(INSTALL_PREFIX)/lib/; fi
	if [ -f ./openarcrt/libopenaccrt_irispf.a ]; then cp -f ./openarcrt/libopenaccrt_irispf.a $(INSTALL_PREFIX)/lib/; fi
	if [ -f ./openarcrt/libopenaccrtomp_irispf.a ]; then cp -f ./openarcrt/libopenaccrtomp_irispf.a $(INSTALL_PREFIX)/lib/; fi
...
help:
	...
	@echo "    - Available options for [target-system]: CURRENT, CUDA, OSX, OPENCL, HIP, IRIS, BRISBANE, INTELFPGA, ..."
