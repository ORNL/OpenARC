...
#endif
    {
        ...
        if(CudaDriver::pinnedHostMemCounter.find(host) == CudaDriver::pinnedHostMemCounter.end() )	{
            ...
            if(cuResult == CUDA_SUCCESS) {
				...
				result = HI_error;
				fprintf(stderr, "[OPENARCRT-WARNING in CudaDriver::pin_host_memory_if_unpinned()] Host memory pinning failed!\n");
            }
        }
    }
#ifdef _THREAD_SAFETY
...
HI_error_t CudaDriver::HI_free( const void *hostPtr, int asyncID, int threadID) {
    ...
    if( HI_get_device_address(hostPtr, &devPtr, NULL, &size, asyncID, tconf->threadID) != HI_error) {
		...
		//HI_free_unified().
		//[DEBUG on Aug. 31, 2021] Skip if devPtr == 0, which is possible when CUDA constant memory is used.
        ...
        if( tempPtr != 0 ) {
			...
			if( tempMallocSize->count((const void *)tempPtr) == 0 ) {
				if( tempMallocSet.count(tempPtr) > 0 ) {
					...
					//fprintf(stderr, "[OPENARCRT-WARNING in CudaDriver::HI_tempFree(devType = %s, thread ID = %d)] no tempMallocSet mapping found for tempPtr (%lx)\n", HI_get_device_type_string(devType), threadID, (long unsigned int)(tempPtr));
				}
			} else {
...
HI_error_t  CudaDriver::HI_memcpy(void *dst, const void *src, size_t count, HI_MemcpyKind_t kind, int trType, int threadID) {
	...
	if( HI_openarcrt_verbosity > 2 ) {
		...
		exit(1);
#else
        fprintf(stderr, "[ERROR in CudaDriver::HI_memcpy()] Memcpy failed with error %d (%s) in tid %d\n", cuResult, cuda_error_code(cuResult), tconf->threadID);
		exit(1);
#endif
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
...
*/
	...
	if( HI_openarcrt_verbosity > 2 ) {
		...
		exit(1);
#else
        fprintf(stderr, "[ERROR in CudaDriver::HI_memcpy_async()] Memcpy failed with error %d (%s) in tid %d with asyncId %d\n", cuResult, cuda_error_code(cuResult), tconf->threadID, async);
		exit(1);
#endif
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
...
HI_error_t CudaDriver::HI_register_kernel_arg(std::string kernel_name, int arg_index, size_t arg_size, void *arg_value, int arg_type, int arg_trait, size_t unitSize, int threadID)
{
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
...
HI_error_t CudaDriver::HI_create_texobj(void *texObj,  HI_datatype_t type, const void *devPtr, size_t size, int threadID) {
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\tenter CudaDriver::HI_create_texobj()\n");
	}
#endif
	if( size > max1DTexRefWidth4LM ) {
        fprintf(stderr, "[ERROR in CudaDriver::HI_create_texobj()] Texture binding for device pointer (%lx) failed due to too large size (%ld); exit!\n", (long unsigned int)devPtr, size);
		exit(1);
	}

    CUresult err;
    CUtexObject* pTexObject = (CUtexObject *)texObj;
    HI_error_t result = HI_success;
	CUDA_RESOURCE_DESC resDesc;
	memset(&resDesc, 0, sizeof(resDesc));
	resDesc.resType = CU_RESOURCE_TYPE_LINEAR;
	resDesc.res.linear.devPtr = (CUdeviceptr)devPtr;
	resDesc.res.linear.sizeInBytes = size;

    if(type == HI_int) {
		resDesc.res.linear.format = CU_AD_FORMAT_SIGNED_INT32;
		resDesc.res.linear.numChannels = 1;
    } else if (type == HI_float) {
		resDesc.res.linear.format = CU_AD_FORMAT_FLOAT;
		resDesc.res.linear.numChannels = 1;
    } else {
        fprintf(stderr, "[ERROR in CudaDriver::HI_create_texobj()] Unsupported format for CUDA texture obj (devPtr = %lx) (NVIDIA CUDA GPU)\n", (long unsigned int)devPtr);
		exit(1);
        result = HI_error;
    }
	...
	err = cuTexObjectCreate(pTexObject, &resDesc, &texDesc, NULL);
    if(err != CUDA_SUCCESS) {
        fprintf(stderr, "[ERROR in CudaDriver::HI_create_texobj()] failed to set format for CUDA texture obj (devPtr = %lx) with error %d (%s)\n", (long unsigned int)devPtr, err, cuda_error_code(err));
		exit(1);
        result = HI_error;
    }
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\texit CudaDriver::HI_create_texobj()\n");
	}
#endif
    return result;
}

HI_error_t CudaDriver::HI_destroy_texobj(void *texObj, const void *devPtr, int threadID) {
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\tenter CudaDriver::HI_destroy_texobj()\n");
	}
#endif
    CUresult err;
    HI_error_t result = HI_success;
	CUtexObject texObject = *((CUtexObject *)texObj);
	
	err = cuTexObjectDestroy(texObject);
    if(err != CUDA_SUCCESS) {
        fprintf(stderr, "[ERROR in CudaDriver::HI_destroy_texobj()] failed to set format for CUDA texture obj (devPtr = %lx) with error %d (%s)\n", (long unsigned int)devPtr, err, cuda_error_code(err));
		exit(1);
        result = HI_error;
    }
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\texit CudaDriver::HI_destroy_texobj()\n");
	}
#endif
    return result;
}
...
#if 0
//[DEBUG on June. 18, 2021] Deprecated on CUDA V11
HI_error_t CudaDriver::HI_bind_texref(std::string texName,  HI_datatype_t type, const void *devPtr, size_t size, int threadID) {
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\tenter CudaDriver::HI_bind_texref()\n");
	}
#endif
	if( size > max1DTexRefWidth4LM ) {
        fprintf(stderr, "[ERROR in CudaDriver::HI_bind_texref()] Texture binding for %s failed due to too large size (%ld); exit!\n",texName.c_str(), size);
		exit(1);
	}
    ...
    if(err != CUDA_SUCCESS) {
        fprintf(stderr, "[ERROR in CudaDriver::HI_bind_texref()] failed to find CUDA texture '%s' with error %d (%s)\n", texName.c_str(), err, cuda_error_code(err));
		exit(1);
    }
	//[DEBUG on Feb. 10, 2021] Deprecated on CUDA V11
    err = cuTexRefSetAddress(0, cuTexref, (CUdeviceptr)devPtr, size);
    if(err != CUDA_SUCCESS) {
        fprintf(stderr, "[ERROR in CudaDriver::HI_bind_texref()] failed to set address for CUDA texture '%s' with error %d (%s)\n", texName.c_str(), err, cuda_error_code(err));
		exit(1);
    }
	//[DEBUG on Feb. 10, 2021] Deprecated on CUDA V11
    err = cuTexRefSetAddressMode(cuTexref, 0, CU_TR_ADDRESS_MODE_WRAP);
    if(err != CUDA_SUCCESS) {
        fprintf(stderr, "[ERROR in CudaDriver::HI_bind_texref()] failed to set address mode for CUDA texture '%s' with error %d (%s)\n", texName.c_str(), err, cuda_error_code(err));
		exit(1);
    }
	//[DEBUG on Feb. 10, 2021] Deprecated on CUDA V11
    err = cuTexRefSetFilterMode(cuTexref, CU_TR_FILTER_MODE_LINEAR);
    if(err != CUDA_SUCCESS) {
        fprintf(stderr, "[ERROR in CudaDriver::HI_bind_texref()] failed to set filter mode for CUDA texture '%s' with error %d (%s)\n", texName.c_str(), err, cuda_error_code(err));
		exit(1);
    }
	//[DEBUG on Feb. 10, 2021] Deprecated on CUDA V11
    err = cuTexRefSetFlags(cuTexref, CU_TRSF_NORMALIZED_COORDINATES);
    if(err != CUDA_SUCCESS) {
        fprintf(stderr, "[ERROR in CudaDriver::HI_bind_texref()] failed to set flags for CUDA texture '%s' with error %d (%s)\n", texName.c_str(), err, cuda_error_code(err));
		exit(1);
    }
    ...
    if(type == HI_int) {
        ...
        fprintf(stderr, "[ERROR in CudaDriver::HI_bind_texref()] Unsupported format for CUDA texture '%s' (NVIDIA CUDA GPU)\n", texName.c_str());
		exit(1);
        result = HI_error;
    }

    if(err != CUDA_SUCCESS) {
        fprintf(stderr, "[ERROR in CudaDriver::HI_bind_texref()] failed to set format for CUDA texture '%s' with error %d (%s)\n", texName.c_str(), err, cuda_error_code(err));
		exit(1);
        result = HI_error;
    }
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\texit CudaDriver::HI_bind_texref()\n");
	}
#endif
    return result;
}
