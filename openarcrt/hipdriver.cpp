...
HI_error_t HipDriver::init(int threadID) {
	...
	//[DEBUG on June 17, 2021 (SL)] Rename HIP_PLATFORM to OPENARCRT_HIP_PLATFORM.
    const char* hip_platform = std::getenv("OPENARCRT_HIP_PLATFORM");
	if( hip_platform == NULL ) {
    	hip_platform = std::getenv("HIP_PLATFORM");
	}
    ...
    if( access( codeName.c_str(), F_OK ) == -1 ) {
      ...
      std::string command;
      // [DEBUG on April 15, 2021 (JL)] set -arch=sm string when HIP_PLATFORM=nvcc
      if (hip_platform != NULL && !strcmp(hip_platform, "nvcc")) {
        command = std::string("hipcc --genco $OPENARC_JITOPTION -arch=sm_") + version + std::string(" ") + fileNameBase + std::string(".hip.cpp -o ") + codeName;
      }
      else {
        command = std::string("hipcc --genco ") + fileNameBase + std::string(".hip.cpp -o ") + codeName;
      }

        system(command.c_str());
    }

	// [DEBUG on July 6, 2021] Below code will be activated only when OPENARCRT_HIP_PLATFORM environment variable is defined.
 #ifdef OPENARCRT_HIP_PLATFORM
    // [DEBUG on April 15, 2021 (JL)] hipCtxCreate() required when HIP_PLATFORM=nvcc
    if (hip_platform != NULL && !strcmp(hip_platform, "nvcc")) {
      hipCtx_t hipContext;
      // CU_CTX_SCHED_BLOCKING_SYNC = 0x04
      err = hipCtxCreate(&hipContext, 0x04, hipDevice);
      if (err != hipSuccess) {
...
HI_error_t HipDriver::HI_malloc1D(const void *hostPtr, void **devPtr, size_t count, int asyncID, HI_MallocKind_t flags, int threadID) {
    ...
    if(HI_get_device_address(hostPtr, devPtr, NULL, NULL, asyncID, tconf->threadID) == HI_success ) {
        ...
        if(HI_get_device_address_from_victim_cache(hostPtr, devPtr, NULL, NULL, asyncID, tconf->threadID) == HI_success ) {
            ...
            if (hipResult != hipSuccess) {
                if( tconf->prepin_host_memory == 1 ) {
                    unpin_host_memory_all(asyncID, tconf->threadID);
                } else {
                ...
                if (hipResult != hipSuccess) {
                    if( tconf->prepin_host_memory == 1 ) {
                        unpin_host_memory_all(tconf->threadID);
                    } else {
...
HI_error_t HipDriver::HI_free( const void *hostPtr, int asyncID, int threadID) {
    ...
    if( HI_get_device_address(hostPtr, &devPtr, NULL, &size, asyncID, tconf->threadID) != HI_error) {
        ...
        //HI_free_unified().
		//[DEBUG on Aug. 31, 2021] Skip if devPtr == 0, which is possible when HIP constant memory is used.
        if( (hostPtr != devPtr) && (devPtr != 0) ) {
#if VICTIM_CACHE_MODE == 1
        ...
        if( tempPtr != 0 ) {
            ...
            if( tempMallocSize->count((const void *)tempPtr) == 0 ) {
                if( tempMallocSet.count(tempPtr) > 0 ) {
                    ...
                    //fprintf(stderr, "[OPENARCRT-WARNING in HipDriver::HI_tempFree(devType = %d, thread ID = %d)] no tempMallocSet mapping found for tempPtr (%lx)\n", devType, threadID, (long unsigned int)(tempPtr));
                }
            } else {
...
HI_error_t HipDriver::HI_memcpy(void *dst, const void *src, size_t count, HI_MemcpyKind_t kind, int trType, int threadID) {
	...
	if( dst != src ) {
    	switch( kind ) {
    		...
    		case HI_MemcpyHostToDevice:     err = hipMemcpyHtoD((hipDeviceptr_t) dst, (void*) src, count);   break;
    		case HI_MemcpyDeviceToHost:     err = hipMemcpyDtoH(dst, (hipDeviceptr_t) src, count);   break;
    		case HI_MemcpyDeviceToDevice:   err = hipMemcpyDtoD((hipDeviceptr_t) dst, (hipDeviceptr_t) src, count);   break;
    	}
	}
#ifdef _OPENARC_PROFILE_
    if( dst != src ) {
...
HI_error_t HipDriver::HI_memcpy_async(void *dst, const void *src, size_t count, HI_MemcpyKind_t kind, int trType, int async, int num_waits, int *waits, int threadID) {
    ...
    if( dst != src ) {
        ...
        case HI_MemcpyHostToDevice: {
            if( tconf->prepin_host_memory == 0 ) {
				size_t wholesize = 0;
            	baseHostPtr = HI_get_base_address_of_host_memory(src, &wholesize, async, tconf->threadID);
            	if( baseHostPtr == 0 ) {
                	baseHostPtr = src;
            	}
				if( wholesize == 0 ) {
            	...
            	pin_host_memory_if_unpinned(baseHostPtr, wholesize, tconf->threadID);
			}
            hipResult = hipMemcpyHtoDAsync( (hipDeviceptr_t) dst, (void *)src, count, stream);
            break;
        }
        case HI_MemcpyDeviceToHost: {
            if( tconf->prepin_host_memory == 0 ) {
				size_t wholesize = 0;
            	baseHostPtr = HI_get_base_address_of_host_memory((const void *)dst, &wholesize, async, tconf->threadID);
            	if( baseHostPtr == 0 ) {
            ...
            hipResult = hipMemcpyDtoHAsync(dst, (hipDeviceptr_t) src, count, stream);
            break;
        }
        case HI_MemcpyDeviceToDevice: {
            hipResult = hipMemcpyDtoDAsync((hipDeviceptr_t) dst, (hipDeviceptr_t) src, count, stream);
            break;
        }
        }
    }
    ...
    if( HI_openarcrt_verbosity > 2 ) {
        ...
        exit(1);
#else
        fprintf(stderr, "[ERROR in HipDriver::HI_memcpy_async()] Memcpy failed with error %d in tid %d with asyncId %d\n", hipResult, tconf->threadID, async);
        exit(1);
#endif
#ifdef _OPENARC_PROFILE_
    if( HI_openarcrt_verbosity > 2 ) {
...
HI_error_t HipDriver::HI_register_kernel_arg(std::string kernel_name, int arg_index, size_t arg_size, void *arg_value, int arg_type, int arg_trait, size_t unitSize, int threadID) {
#ifdef _OPENARC_PROFILE_
    if( HI_openarcrt_verbosity > 2 ) {
...
HI_error_t HipDriver::HI_bind_texref(std::string texName,  HI_datatype_t type, const void *devPtr, size_t size, int threadID) {
#ifdef _OPENARC_PROFILE_
    if( HI_openarcrt_verbosity > 2 ) {
        fprintf(stderr, "[OPENARCRT-INFO]\t\tenter HipDriver::HI_bind_texref()\n");
    }
#endif
/*
    if( size > max1DTexRefWidth4LM ) {
        fprintf(stderr, "[ERROR in HipDriver::HI_bind_texref()] Texture binding for %s failed due to too large size (%ld); exit!\n",texName.c_str(), size);
        exit(1);
    }
*/
...
//[DEBUG on Feb. 17, 2022] the texture memory support is disabled, since the texture memory support of the HIP runtime is deprecated.
	HI_error_t result = HI_success;
/*
	hipError_t err;
	textureReference* hipTexref;
	err = hipModuleGetTexRef(&hipTexref, hipModule, texName.c_str());
	if(err != hipSuccess) {
		fprintf(stderr, "[ERROR in HipDriver::HI_bind_texref()] failed to find HIP texture '%s' with error %d \n", texName.c_str(), err);
		exit(1);
	}
	err = hipTexRefSetAddress(0, hipTexref, (hipDeviceptr_t)devPtr, size);
	if(err != hipSuccess) {
		fprintf(stderr, "[ERROR in HipDriver::HI_bind_texref()] failed to set address for HIP texture '%s' with error %d\n", texName.c_str(), err);
		exit(1);
	}
	err = hipTexRefSetAddressMode(hipTexref, 0, hipAddressModeWrap);
	if(err != hipSuccess) {
		fprintf(stderr, "[ERROR in HipDriver::HI_bind_texref()] failed to set address mode for HIP texture '%s' with error %d \n", texName.c_str(), err);
		exit(1);
	}
	err = hipTexRefSetFilterMode(hipTexref, hipFilterModePoint);
	if(err != hipSuccess) {
		fprintf(stderr, "[ERROR in HipDriver::HI_bind_texref()] failed to set filter mode for HIP texture '%s' with error %d \n", texName.c_str(), err);
		exit(1);
	}
	err = hipTexRefSetFlags(hipTexref, 0);
	if(err != hipSuccess) {
		fprintf(stderr, "[ERROR in HipDriver::HI_bind_texref()] failed to set flags for HIP texture '%s' with error %d \n", texName.c_str(), err);
		exit(1);
	}

	if(type == HI_int) {
		err = hipTexRefSetFormat(hipTexref, HIP_AD_FORMAT_SIGNED_INT32, 1);
	} else if (type == HI_float) {
		err = hipTexRefSetFormat(hipTexref, HIP_AD_FORMAT_FLOAT, 1);
	} else {
		fprintf(stderr, "[ERROR in HipDriver::HI_bind_texref()] Unsupported format for HIP texture '%s' (AMD HIP GPU)\n", texName.c_str());
		exit(1);
		result = HI_error;
	}

	if(err != hipSuccess) {
		fprintf(stderr, "[ERROR in HipDriver::HI_bind_texref()] failed to set format for HIP texture '%s' with error %d \n", texName.c_str(), err);
		exit(1);
		result = HI_error;
	}
*/
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\texit HipDriver::HI_bind_texref()\n");
	}
#endif
	return result;
}

HI_error_t HipDriver::HI_create_texobj(void * texObj,  HI_datatype_t type, const void *devPtr, size_t size, int threadID) {
#ifdef PRINT_TODO
	fprintf(stderr, "[%s:%d][%s] Not Implemented!\n", __FILE__, __LINE__, __func__);
#endif
...
HI_error_t HipDriver::HI_destroy_texobj(void * texObj,  const void *devPtr, int threadID) {
#ifdef PRINT_TODO
	fprintf(stderr, "[%s:%d][%s] Not Implemented!\n", __FILE__, __LINE__, __func__);
#endif
...
HI_error_t HipDriver::HI_memcpy_const(void *hostPtr, std::string constName, HI_MemcpyKind_t kind, size_t count, int threadID) {
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\tenter HipDriver::HI_memcpy_const(%lu)\n", count);
	}
#endif
    HostConf_t * tconf = getHostConf(threadID);
	hipError_t hipResult;
    ...
    hipResult = hipModuleGetGlobal( &dptr, &size, hipModule, constName.c_str());
...
//#endif
    ...
    if( hipResult != hipSuccess ) {
#ifdef _OPENMP
        fprintf(stderr, "[ERROR in HipDriver::HI_memcpy_const()] Acquiring constant memory handle failed with error %d in tid %d\n", hipResult, omp_get_thread_num());
		exit(1);
#else
        fprintf(stderr, "[ERROR in HipDriver::HI_memcpy_const()] Acquiring constant memory handle failed with error %d in tid %d\n", hipResult, tconf->threadID);
		exit(1);
#endif
        result = HI_error;
    }
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\texit HipDriver::HI_memcpy_const(%lu)\n", count);
	}
#endif
    return result;
}

//[DEBUG] HIP driver does not offer asynchronous version of hipModuleGetGlobal(), 
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\tenter HipDriver::HI_memcpy_const_async(%lu)\n", count);
	}
#endif
    HostConf_t * tconf = getHostConf(threadID);
	hipError_t hipResult;
...
//#endif
    ...
    if( hipResult != hipSuccess ) {
#ifdef _OPENMP
        fprintf(stderr, "[ERROR in HipDriver::HI_memcpy_const_async()] Acquiring constant memory handle failed with error %d in tid %d\n", hipResult, omp_get_thread_num());
		exit(1);
#else
        fprintf(stderr, "[ERROR in HipDriver::HI_memcpy_const_async()] Acquiring constant memory handle failed with error %d in tid %d\n", hipResult, tconf->threadID);
		exit(1);
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\texit HipDriver::HI_memcpy_const_async(%lu)\n", count);
	}
#endif
    return result;
}
