...
int acc_get_num_devices( acc_device_t devtype, int threadID ) {
    ...
    if( (devtype == acc_device_not_host) || (devtype == acc_device_default) ) {
        ...
        count = BrisbaneDriver::HI_get_num_devices(tconf->acc_device_type_var);
#else
        count = IrisDriver::HI_get_num_devices(tconf->acc_device_type_var);
#endif
#else
		...
		count = CudaDriver::HI_get_num_devices(tconf->acc_device_type_var);
#endif
    } else if( (devtype == acc_device_nvidia) || (devtype == acc_device_radeon)|| (devtype == acc_device_intelgpu) ) {
#if defined(OPENARC_ARCH) && OPENARC_ARCH != 0 && OPENARC_ARCH != 5 && OPENARC_ARCH != 6
...
#else
        count = IrisDriver::HI_get_num_devices(devtype);
#endif
#else
		//[DEBUG on April 1, 2021] Changed to use user-provided device type as it is.
        //devtype = acc_device_gpu;
        ...
        count = 0;
#endif
    } else if( (devtype == acc_device_altera) || (devtype == acc_device_altera_emulator) ) {
...
void acc_set_device_type( acc_device_t devtype, int threadID ) {
    ...
    if( devtype == acc_device_nvidia || devtype == acc_device_radeon || devtype == acc_device_gpu ) {
...
#elif defined(OPENARC_ARCH) && OPENARC_ARCH == 6
        tconf->acc_device_type_var = acc_device_default;
#else
        tconf->acc_device_type_var = acc_device_gpu;
#endif
        tconf->isOnAccDevice = 1;
        ...
        tconf->acc_device_type_var = acc_device_altera_emulator;
        tconf->isOnAccDevice = 1;
    } else if ( devtype == acc_device_xeonphi ) {
        tconf->acc_device_type_var = acc_device_xeonphi;
        tconf->isOnAccDevice = 1;
...
//    acc_device_default 			#for IRIS/Brisbane devices
...
void acc_set_device_num( int devnum, acc_device_t devtype, int threadID ) {
	...
	//When targeting IRIS/Brisbane devices, always use acc_device_type_default
	//so that the IRIS/Brisbane runtime can choose appropriate device.
	//Or, user can choose by setting the device number.
	//[DEBUG on April 1, 2021] We no longer change the device type to  acc_device_default type for IRIS/Brisbane devices;
	//instead, use whatever the user provides.
	//[DEBUG on May 13, 2021] When  targeting IRIS/Brisbane devices, assume that there is only one logical IRIS/Brisbane device of 
	//acc_device_default type, which will internally choose actual devices based on the user-provided device type or policy.
	current_devtype = acc_device_default;
	alter_devtype = acc_device_default;
	devnum = 0;
#endif
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 0 ) {
		fprintf(stderr, "[OPENARCRT-INFO] exit acc_set_device_num(devnum = %d, devtype = %s, thread ID = %d)\n", tconf->acc_device_num_var, HI_get_device_type_string(tconf->acc_device_type_var), threadID);
	}
#endif
}
...
void acc_init( acc_device_t devtype, int kernels, std::string kernelNames[], int defaultNumAsyncQueues, const char *fileNameBase, int threadID ) {
#ifdef _OPENARC_PROFILE_
    if( HI_hostinit_done == 0 ) {
		int openarcrt_verbosity;
		char * envVar;
	...
	omp_helper_set_queue_max(defaultNumAsyncQueues);
...
void acc_init( acc_device_t devtype, int kernels, std::string kernelNames[], int defaultNumAsyncQueues, const char *fileNameBase) {
	acc_init( devtype, kernels, kernelNames, defaultNumAsyncQueues, fileNameBase, NO_THREAD_ID);
}
...
int acc_on_device( acc_device_t devtype, int threadID ) {
    ...
    if( (devtype == acc_device_nvidia) || (devtype == acc_device_not_host) ||
			...
			|| (devtype == acc_device_gpu) || (devtype == acc_device_altera_emulator) ) {
...
void acc_free(void* devPtr, int threadID) {
	...
	void *devPtr2;
	size_t memSize = 0;
	tconf->device->HI_get_device_address(devPtr, &devPtr2, NULL, &memSize, DEFAULT_QUEUE+tconf->asyncID_offset, tconf->threadID);
#endif
    if( tconf->isOnAccDevice ) {
		if(tconf->device == NULL) {
	...
	if( memSize > 0 ) {
		...
		fprintf(stderr, "[OPENARCRT-INFO] acc_free(thread ID = %d) has non-positive size, %lu\n",threadID, memSize);
	}
	if( HI_openarcrt_verbosity > 0 ) {
...
void acc_update_device_async(h_void* hostPtr, size_t size, int async, int threadID) {
	void* devPtr;
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 0 ) {
		fprintf(stderr, "[OPENARCRT-INFO] enter acc_update_device_async(hostPtr = %lx, size = %ld, async = %d, thread ID = %d)\n",(long unsigned int)hostPtr, size, async, threadID);
	}
#endif
    //HostConf_t * tconf = getHostConf(threadID);
    if( threadID == NO_THREAD_ID ) {
	...
	if ((HI_get_device_address(hostPtr, &devPtr, DEFAULT_QUEUE, threadID)==HI_success)) {
		...
		fprintf(stderr, "[OPENARCRT-ERROR] the argument data of acc_update_device_async(hostPtr = %lx, size = %ld, async = %d, thread ID = %d) is not present on the device; exit!\n",(long unsigned int)hostPtr, size, async, threadID);
		exit(1);
	}
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 0 ) {
		fprintf(stderr, "[OPENARCRT-INFO] exit acc_update_device_async(hostPtr = %lx, size = %ld, async = %d, thread ID = %d)\n",(long unsigned int)hostPtr, size, async, threadID);
	}
#endif
}

void acc_update_device_async(h_void* hostPtr, size_t size, int async) {
	acc_update_device_async(hostPtr, size, async, NO_THREAD_ID);
}
...
void acc_update_self_async(h_void* hostPtr, size_t size, int async, int threadID) {
	void* devPtr;
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 0 ) {
		fprintf(stderr, "[OPENARCRT-INFO] enter acc_update_self_async(hostPtr = %lx, size = %ld, async = %d, thread ID = %d)\n",(long unsigned int)hostPtr, size, async, threadID);
	}
#endif
    //HostConf_t * tconf = getHostConf(threadID);
    if( threadID == NO_THREAD_ID ) {
	...
	if ((HI_get_device_address(hostPtr, &devPtr, DEFAULT_QUEUE, threadID)==HI_success)) {
		...
		fprintf(stderr, "[OPENARCRT-ERROR] the argument data of acc_update_self_async(hostPtr = %lx, size = %ld, async = %d, thread ID = %d) is not present on the device; exit!\n",(long unsigned int)hostPtr, size, async, threadID);
		exit(1);
	}
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 0 ) {
		fprintf(stderr, "[OPENARCRT-INFO] exit acc_update_self_async(hostPtr = %lx, size = %ld, async = %d, thread ID = %d)\n",(long unsigned int)hostPtr, size, async, threadID);
	}
#endif
}

void acc_update_self_async(h_void* hostPtr, size_t size, int async) {
	acc_update_self_async(hostPtr, size, async, NO_THREAD_ID);
}
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 0 ) {
		fprintf(stderr, "[OPENARCRT-INFO] enter acc_map_data(hostPtr = %lx, devPtr = %lx, size = %ld, thread ID = %d)\n",(long unsigned int)hostPtr, (long unsigned int)devPtr,  size, threadID);
	}
#endif
    //HostConf_t * tconf = getHostConf(threadID);
    if( threadID == NO_THREAD_ID ) {
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 0 ) {
		fprintf(stderr, "[OPENARCRT-INFO] exit acc_map_data(hostPtr = %lx, devPtr = %lx, size = %ld, thread ID = %d)\n",(long unsigned int)hostPtr, (long unsigned int)devPtr,  size, threadID);
	}
#endif
}
...
void acc_map_data_forced(h_void* hostPtr, d_void* devPtr, size_t size, int threadID) {
	void* tDevPtr;
	void* tHostPtr;
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 0 ) {
		fprintf(stderr, "[OPENARCRT-INFO] enter acc_map_data_forced(hostPtr = %lx, devPtr = %lx, size = %ld, thread ID = %d)\n",(long unsigned int)hostPtr, (long unsigned int)devPtr,  size, threadID);
	}
#endif
    //HostConf_t * tconf = getHostConf(threadID);
    if( threadID == NO_THREAD_ID ) {
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 0 ) {
		fprintf(stderr, "[OPENARCRT-INFO] exit acc_map_data_forced(hostPtr = %lx, devPtr = %lx, size = %ld, thread ID = %d)\n",(long unsigned int)hostPtr, (long unsigned int)devPtr,  size, threadID);
	}
#endif
}

void acc_map_data_forced(h_void* hostPtr, d_void* devPtr, size_t size) {
	acc_map_data_forced(hostPtr, devPtr, size, NO_THREAD_ID);
}
