...
#else
            	numDevices = IrisDriver::HI_get_num_devices_init(ttDevType);
#endif
#endif
        		}
...
#else
                		dev = new IrisDriver_t(ttDevType, i, kernelnames, this, numDevices, baseFileName.c_str());
#endif
#endif
            		}
...
#else
    printf("Number of IRIS Task Submissions: %ld\n", BTaskCnt);
#endif
...
HI_error_t HI_register_kernel_arg(std::string kernel_name, int arg_index, size_t arg_size, void *arg_value, int arg_type, int arg_trait, size_t unitSize, int threadID)
{
	HI_error_t return_status;
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 1 ) {
...
#endif
    HostConf_t* tconf = getHostConf(threadID);
    return_status = tconf->device->HI_register_kernel_arg(kernel_name, arg_index, arg_size, arg_value, arg_type, arg_trait, unitSize, tconf->threadID);
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 1 ) {
...
void HI_tempMalloc1D( void** tempPtr, size_t count, acc_device_t devType, HI_MallocKind_t flags, int threadID) {
    ...
    devType == acc_device_altera || devType == acc_device_altera_emulator ||
    devType == acc_device_current || devType == acc_device_intelgpu ) {
		tconf->DMallocCnt++;
		tconf->DMallocSize += count;
    	tconf->CDMemorySize += count;
    	if( tconf->MDMemorySize < tconf->CDMemorySize ) {
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 1 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\tenter HI_tempFree_async(tempPtr = %lx, devType = %s, asyncID = %d, thread ID = %d)\n", (long unsigned int)(*tempPtr), HI_get_device_type_string(devType), asyncID, threadID);
	}
#endif
    HostConf_t * tconf = getHostConf(threadID);
	if(tconf->device == NULL) {
    ...
    devType == acc_device_current || devType == acc_device_intelgpu ) {
        ...
        if( tconf->device->tempMallocSizeMap.count(tconf->threadID) > 0 ) {
        	...
        	if( tempMallocSize->count(*tempPtr) > 0 ) {
				...
				fprintf(stderr, "[OPENARCRT-INFO]\tHI_tempFree_async(tempPtr = %lx, devType = %s, asyncID = %d, thread ID = %d) does not know data size: case 1\n",(long unsigned int)(*tempPtr), HI_get_device_type_string(devType), asyncID, threadID);
			}
        } else {
			fprintf(stderr, "[OPENARCRT-INFO]\tHI_tempFree_async(tempPtr = %lx, devType = %s, asyncID = %d, thread ID = %d) does not know data size: case 2\n",(long unsigned int)(*tempPtr), HI_get_device_type_string(devType), asyncID, threadID);
		}
	} else {
		tconf->HFreeCnt++;
	}
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 1 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\texit HI_tempFree_async(tempPtr = %lx, devType = %s, asyncID = %d, thread ID = %d)\n", (long unsigned int)(*tempPtr), HI_get_device_type_string(devType), asyncID, threadID);
	}
#endif
}
...
void HI_check_read(const void * hostPtr, acc_device_t dtype, const char * varName, const char *refName, int loopIndex, int threadID) {
    ...
    if( dtype == acc_device_nvidia || (dtype == acc_device_radeon) || (dtype == acc_device_gpu) || (dtype == acc_device_xeonphi) || 
	(dtype == acc_device_altera) || (dtype == acc_device_altera_emulator) || (dtype == acc_device_intelgpu) ) {
...
HI_error_t HI_bind_texref(std::string texName,  HI_datatype_t type, const void *devPtr, size_t size, int threadID) {
	HI_error_t return_status;
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 1 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\tenter HI_bind_texref(thread ID = %d)\n", threadID);
	}
#endif
    HostConf_t * tconf = getHostConf(threadID);
	if(tconf->device == NULL) {
        fprintf(stderr, "[ERROR in HI_bind_texref()] Not supported in the current device type %d; exit!\n", tconf->acc_device_type_var);
		exit(1);
	}
    return_status = tconf->device->HI_bind_texref(texName, type, devPtr, size, tconf->threadID);
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 1 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\texit HI_bind_texref(thread ID = %d)\n", threadID);
	}
#endif
	return return_status;
}

HI_error_t HI_create_texobj(void *texObj,  HI_datatype_t type, const void *devPtr, size_t size, int threadID) {
	HI_error_t return_status;
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 1 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\tenter HI_create_texobj(thread ID = %d)\n", threadID);
	}
#endif
    HostConf_t * tconf = getHostConf(threadID);
	if(tconf->device == NULL) {
        fprintf(stderr, "[ERROR in HI_create_texobj()] Not supported in the current device type %d; exit!\n", tconf->acc_device_type_var);
		exit(1);
	}
    return_status = tconf->device->HI_create_texobj(texObj, type, devPtr, size, tconf->threadID);
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 1 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\texit HI_create_texobj(thread ID = %d)\n", threadID);
	}
#endif
	return return_status;
}

HI_error_t HI_destroy_texobj(void *texObj,  const void *devPtr, int threadID) {
	HI_error_t return_status;
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 1 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\tenter HI_destroy_texobj(thread ID = %d)\n", threadID);
	}
#endif
    HostConf_t * tconf = getHostConf(threadID);
	if(tconf->device == NULL) {
        fprintf(stderr, "[ERROR in HI_destroy_texobj()] Not supported in the current device type %d; exit!\n", tconf->acc_device_type_var);
		exit(1);
	}
    return_status = tconf->device->HI_destroy_texobj(texObj, devPtr, tconf->threadID);
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 1 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\texit HI_destroy_texobj(thread ID = %d)\n", threadID);
	}
#endif
	return return_status;
}
