...
//[DEBUG on Oct. 1, 2021] change to 2GB for 32 bit OS long type.
#define _MAX_MEMPOOL_SIZE 2147483647
...
#else
typedef std::map<Accelerator *, std::map<std::string, iris_kernel> > kernelmapiris_t;
#endif
#else
...
#if defined(OPENARC_ARCH) && OPENARC_ARCH == 6
#if defined(OPENARCRT_USE_BRISBANE) && OPENARCRT_USE_BRISBANE == 1
typedef std::map<int, int> threadasyncmapbrisbane_t;
typedef std::map<int, brisbane_task> threadtaskmapbrisbane_t;
typedef std::map<int, threadtaskmapbrisbane_t *> threadasynctaskmapbrisbane_t;
...
public:
    ...
    HI_error_t HI_register_kernel_arg(std::string kernel_name, int arg_index, size_t arg_size, void *arg_value, int arg_type, int arg_trait, size_t unitSize, int threadID=NO_THREAD_ID);
    ...
    HI_error_t HI_bind_texref(std::string texName,  HI_datatype_t type, const void *devPtr, size_t size, int threadID=NO_THREAD_ID);
    HI_error_t HI_create_texobj(void *texObj,  HI_datatype_t type, const void *devPtr, size_t size, int threadID=NO_THREAD_ID);
    HI_error_t HI_destroy_texobj(void *texObj,  const void *devPtr, int threadID=NO_THREAD_ID);
...
public:
	...
	threadasyncmapbrisbane_t threadAsyncMap;
	threadtaskmapbrisbane_t threadTaskMap;
	threadasynctaskmapbrisbane_t threadAsyncTaskMap;
...
public:
    IrisDriver(acc_device_t devType, int devNum, std::set<std::string>kernelNames, HostConf_t *conf, int numDevices, const char *baseFileName);
    HI_error_t init(int threadID=NO_THREAD_ID);
    HI_error_t HI_register_kernels(std::set<std::string>kernelNames, int threadID=NO_THREAD_ID);
    HI_error_t HI_register_kernel_numargs(std::string kernel_name, int num_args, int threadID=NO_THREAD_ID);
    HI_error_t HI_register_kernel_arg(std::string kernel_name, int arg_index, size_t arg_size, void *arg_value, int arg_type, int arg_trait, size_t unitSize, int threadID=NO_THREAD_ID);
