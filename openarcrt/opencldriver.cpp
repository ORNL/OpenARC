...
HI_error_t OpenCLDriver::init(int threadID) {
    ...
    char *platformName;
	acc_device_t devType = dev;
	cl_device_id devices[num_devices];
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\tenter OpenCLDriver::init(%d, %s)\n", device_num, HI_get_device_type_string(devType));
	}
#endif
	...
	for( unsigned i=0; i<num_platforms; i++ ) {
        ...
        if(devType == acc_device_altera) {
			...
			if( name.find(search) == std::string::npos ) {
				...
				err = clGetDeviceIDs(clPlatform, CL_DEVICE_TYPE_ALL, num_devices, devices, &numDevices);
		...
		} else if(devType == acc_device_altera_emulator) {
			search = "emulation";
			if( name.find(search) == std::string::npos ) {
				delete [] namestr;
				platformName = NULL;
			...
			if( name.find(search) == std::string::npos ) {
				...
				err = clGetDeviceIDs(clPlatform, CL_DEVICE_TYPE_ACCELERATOR, num_devices, devices, &numDevices);
        		if ( (err != CL_SUCCESS) || (numDevices <= 0) ) {
					delete [] namestr;
					platformName = NULL;
					continue;
			...
			if( name.find(search) == std::string::npos ) {
				...
				err = clGetDeviceIDs(clPlatform, CL_DEVICE_TYPE_GPU, num_devices, devices, &numDevices);
        		if ( (err != CL_SUCCESS) || (numDevices <= 0) ) {
					delete [] namestr;
					platformName = NULL;
					continue;
		...
		} else if( devType == acc_device_nvidia ) {
			search = "nvidia";
			if( name.find(search) == std::string::npos ) {
				delete [] namestr;
				platformName = NULL;
		...
		} else if( devType == acc_device_radeon ) {
			search = "amd";
			if( name.find(search) == std::string::npos ) {
				search = "advanced";
			}
			...
			err = clGetDeviceIDs(clPlatform, CL_DEVICE_TYPE_CPU, num_devices, devices, &numDevices);
        	if ( (err != CL_SUCCESS) || (numDevices <= 0) ) {
				delete [] namestr;
				platformName = NULL;
				continue;
    ...
    if ( !foundPlatform ) {
		fprintf(stderr, "[ERROR in OpenCLDriver::init()] Failed to get device IDs for device type %s\n", HI_get_device_type_string(devType));
        if (err != CL_SUCCESS) {
...
#ifdef _OPENARC_PROFILE_
    fprintf(stderr, "[OPENARCRT-INFO]\t\tOpenCLDriver::init(): host Thread %d initializes device %d: %s\n", thread_id, device_num, cBuffer1);
#endif
    fp = fopen(filename, "r");
    if (!fp) {
    	if(dev != acc_device_altera || dev != acc_device_altera_emulator) {
...
#pragma omp critical(clContext_critical)
    ...
    if(dev == acc_device_altera || dev == acc_device_altera_emulator) {
    	...
    	binaryName = fileNameBase + std::string("_") + cBufferN + std::string(".clbin");
	}
    ...
    if( access( binaryName.c_str(), F_OK ) == -1 ) {
        ...
        size_t readSize = fread(programBinary, 1, binarySize, fp);
		if( readSize != binarySize ) {
            fprintf(stderr, "[ERROR in OpenCLDriver::init()] error in reading the program binary!\n");
			exit(1);
		}
        fclose(fp);
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\tenter OpenCLDriver::HI_get_num_devices(devType = %s)\n", HI_get_device_type_string(devType));
	}
#endif
    cl_uint numDevices = 0;
    if(devType == acc_device_gpu || devType == acc_device_nvidia || devType == acc_device_radeon ||
		devType == acc_device_xeonphi || devType == acc_device_intelgpu ||
		devType == acc_device_altera || devType == acc_device_altera_emulator || devType == acc_device_host ) {
        ...
        if ( !foundPlatform ) {
            fprintf(stderr, "[ERROR in OpenCLDriver::HI_get_num_devices()] Failed to get device IDs for device type %s\n", HI_get_device_type_string(devType));
        	if (err != CL_SUCCESS) {
            	fprintf(stderr, "                                              OpenCL error (%d, %s)\n", err, opencl_error_code(err));
			}
			if(platformName != NULL) {
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\texit OpenCLDriver::HI_get_num_devices(devType = %s); number of devices = %d\n", HI_get_device_type_string(devType), numDevices);
	}
#endif
...
HI_error_t  OpenCLDriver::HI_malloc1D(const void *hostPtr, void **devPtr, size_t count, int asyncID, HI_MallocKind_t flags, int threadID) {
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\tenter OpenCLDriver::HI_malloc1D(hostPtr = %lx, asyncID = %d, size = %lu, flags = %d, thread ID = %d)\n",(long unsigned int)hostPtr, asyncID, count, flags, threadID);
	}
#endif
    HostConf_t * tconf = getHostConf(threadID);
    ...
    if(HI_get_device_address(hostPtr, devPtr, NULL, NULL, asyncID, tconf->threadID) == HI_success ) {
		...
		if( it != memPool->end()) {
			...
			*devPtr = it->second;
        	...
        	memHandle = clCreateBuffer(clContext, mem_flags, count, NULL, &err);
#ifdef _OPENARC_PROFILE_
			tconf->IDMallocCnt++;
        	...
        	if(err != CL_SUCCESS) {
				...
				for (it = memPool->begin(); it != memPool->end(); ++it) {
					...
					if( HI_get_device_mem_handle(tDevPtr, &tHandle, tconf->threadID) == HI_success ) { 
        				err = clReleaseMemObject((cl_mem)(tHandle.memHandle));
#ifdef _OPENARC_PROFILE_
						tconf->IDFreeCnt++;
#endif
        				if(err != CL_SUCCESS) {
        	...
        	if(err == CL_SUCCESS) {
#if defined(OPENARC_ARCH) && OPENARC_ARCH == 3
				if( HI_openarcrt_memoryalignment > 0 ) {
	...
	if( HI_openarcrt_verbosity > 2 ) {
		...
		fprintf(stderr, "[OPENARCRT-INFO]\t\texit OpenCLDriver::HI_malloc1D(hostPtr = %lx, asyncID = %d, size = %lu, flags = %d, thread ID = %d)\n",(long unsigned int)hostPtr, asyncID, count, flags, threadID);
	}
#endif
    return result;
...
/*
			...
			if( HI_get_device_mem_handle(devPtr, &tHandle, tconf->threadID) == HI_success ) { 
        		cl_int  err = clReleaseMemObject((cl_mem)(tHandle.memHandle));
        		if( err == CL_SUCCESS ) {
            		HI_remove_device_address(hostPtr, asyncID, tconf->threadID);
					free(devPtr);
...
void OpenCLDriver::HI_tempMalloc1D( void** tempPtr, size_t count, acc_device_t devType, HI_MallocKind_t flags, int threadID) {
    ...
    devType == acc_device_not_host || devType == acc_device_intelgpu || 
	devType == acc_device_altera || devType == acc_device_altera_emulator || devType == acc_device_current) {
        ...
        cl_int err;
		cl_mem_flags mem_flags = convert2CLMemFlags(flags);
        cl_mem memHandle = clCreateBuffer(clContext, mem_flags, count, NULL, &err);
#if defined(OPENARC_ARCH) && OPENARC_ARCH == 3
		if( HI_openarcrt_memoryalignment > 0 ) {
...
//malloc used for allocating temporary data.
void OpenCLDriver::HI_tempMalloc1D_async( void** tempPtr, size_t count, acc_device_t devType, HI_MallocKind_t flags, int threadID) {
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\tenter OpenCLDriver::HI_tempMalloc1D_async()\n");
	}
#endif
    HostConf_t * tconf = getHostConf(threadID);
#ifdef _OPENARC_PROFILE_
	...
	devType == acc_device_altera || devType == acc_device_altera_emulator || devType == acc_device_current) {
		...
		*tempPtr = malloc(count);
		...
		tempMallocSet.insert(*tempPtr);
        if (err != CL_SUCCESS) {
            fprintf(stderr, "[ERROR in OpenCLDriver::HI_tempMalloc1D_async()] : Malloc failed\n");
			exit(1);
        }
#ifdef _OPENARC_PROFILE_
        tconf->IDMallocCnt++;
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\texit OpenCLDriver::HI_tempMalloc1D_async()\n");
	}
#endif
}
...
//malloc used for allocating temporary data.
void OpenCLDriver::HI_tempMalloc1D_async( void** tempPtr, size_t count, acc_device_t devType, HI_MallocKind_t flags, int asyncID, int threadID) {
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		fprintf(stderr, "[OPENARCRT-INFO]\t\tenter OpenCLDriver::HI_tempMalloc1D_async()\n");
	}
...
#ifdef _OPENARC_PROFILE_
            if( HI_openarcrt_verbosity > 2 ) {
                fprintf(stderr, "[OPENARCRT-INFO]\t\tOpenCLDriver::HI_tempMalloc1D_async(%lu, %d) reuses memories in the memPool\n", count, flags);
            }    
#endif
            *tempPtr = it->second;
            memPool->erase(it);
...
#ifdef _OPENARC_PROFILE_
                if( HI_openarcrt_verbosity > 2 ) {
                    fprintf(stderr, "[OPENARCRT-INFO]\t\tOpenCLDriver::HI_tempMalloc1D_async(%lu, %d) releases memories in the memPool\n", count, flags);
                }
#endif               
                HI_device_mem_handle_t tHandle;
                void * tDevPtr;
...
#endif                       
                        if(err != CL_SUCCESS) {
                            fprintf(stderr, "[ERROR in OpenCLDriver::HI_tempMalloc1D_async()] : failed to free on OpenCL\n");        
                        }
                        free(tDevPtr);
                        HI_remove_device_mem_handle(tDevPtr, tconf->threadID);
                    }        
...
/*
					...
					if( (prefix != AOCL_ALIGNMENT) && (prefix < count) ) {
        				err = clEnqueueWriteBuffer(queue, (cl_mem)(tHandle.memHandle), CL_TRUE, tHandle.offset, prefix, src, 0, NULL, NULL);
        				err = clEnqueueWriteBuffer(queue, (cl_mem)(tHandle.memHandle), CL_TRUE, (tHandle.offset + prefix), count-prefix, (const void *)((char*)(src) + prefix), 0, NULL, NULL);
					} else {
        				err = clEnqueueWriteBuffer(queue, (cl_mem)(tHandle.memHandle), CL_TRUE, tHandle.offset, count, src, 0, NULL, NULL);
					}
*/
					...
					cl_mem mem = (cl_mem)tHandle.memHandle;
...
/*
					...
					if( (prefix != AOCL_ALIGNMENT) && (prefix < count) ) {
        				err = clEnqueueReadBuffer(queue, (cl_mem)(tHandle.memHandle), CL_TRUE, tHandle.offset, prefix, dst, 0, NULL, NULL);
        				err = clEnqueueReadBuffer(queue, (cl_mem)(tHandle.memHandle), CL_TRUE, (tHandle.offset + prefix), count-prefix, (void *)((char*)(dst) + prefix), 0, NULL, NULL);
					} else {
        				err = clEnqueueReadBuffer(queue, (cl_mem)(tHandle.memHandle), CL_TRUE, tHandle.offset, count, dst, 0, NULL, NULL);
					}
*/
					size_t offset = tHandle.offset;
    	...
    	case HI_MemcpyHostToDevice: {
			...
			if( HI_get_device_mem_handle(dst, &tHandle, tconf->threadID) == HI_success ) {
				...
				if( HI_openarcrt_memoryalignment > 0 ) {
					size_t prefix = AOCL_ALIGNMENT - ((size_t) src & (AOCL_ALIGNMENT - 1));
					if( (prefix != AOCL_ALIGNMENT) && (prefix < count) ) {
        				err = clEnqueueWriteBuffer(queue, (cl_mem)(tHandle.memHandle), CL_FALSE, tHandle.offset, prefix, src, 0, NULL, event);
        				err = clEnqueueWriteBuffer(queue, (cl_mem)(tHandle.memHandle), CL_FALSE, (tHandle.offset + prefix), count-prefix, (const void *)((char*)(src) + prefix), 0, NULL, event);
					} else {
        				err = clEnqueueWriteBuffer(queue, (cl_mem)(tHandle.memHandle), CL_FALSE, tHandle.offset, count, src, 0, NULL, event);
					}
				} else {
        			err = clEnqueueWriteBuffer(queue, (cl_mem)(tHandle.memHandle), CL_FALSE, tHandle.offset, count, src, 0, NULL, event);
				}
    	...
    	case HI_MemcpyDeviceToHost: {
			...
			if( HI_get_device_mem_handle(src, &tHandle, tconf->threadID) == HI_success ) {
				...
				if( HI_openarcrt_memoryalignment > 0 ) {
					size_t prefix = AOCL_ALIGNMENT - ((size_t) dst & (AOCL_ALIGNMENT - 1));
					if( (prefix != AOCL_ALIGNMENT) && (prefix < count) ) {
        				err = clEnqueueReadBuffer(queue, (cl_mem)(tHandle.memHandle), CL_FALSE, tHandle.offset, prefix, dst, 0, NULL, event);
        				err = clEnqueueReadBuffer(queue, (cl_mem)(tHandle.memHandle), CL_FALSE, (tHandle.offset + prefix), count-prefix, (void *)((char*)(dst) + prefix), 0, NULL, event);
					} else {
        				err = clEnqueueReadBuffer(queue, (cl_mem)(tHandle.memHandle), CL_FALSE, tHandle.offset, count, dst, 0, NULL, event);
					}
				} else {
        			err = clEnqueueReadBuffer(queue, (cl_mem)(tHandle.memHandle), CL_FALSE, tHandle.offset, count, dst, 0, NULL, event);
				}
...
HI_error_t OpenCLDriver::HI_memcpy_asyncS(void *dst, const void *src, size_t count,
    ...
    case HI_MemcpyHostToDevice: {
		...
		HI_device_mem_handle_t tHandle;
		if( HI_get_device_mem_handle(dst, &tHandle, tconf->threadID) == HI_success ) {
        	err = clEnqueueWriteBuffer(queue, (cl_mem)(tHandle.memHandle), CL_FALSE, tHandle.offset, count, tSrc, 0, NULL, event);
		} else {
    ...
    case HI_MemcpyDeviceToHost: {
		...
		HI_device_mem_handle_t tHandle;
		if( HI_get_device_mem_handle(src, &tHandle, tconf->threadID) == HI_success ) {
        	err = clEnqueueReadBuffer(queue, (cl_mem)(tHandle.memHandle), CL_FALSE, tHandle.offset, count, tDst, 0, NULL, event);
		} else {
        	fprintf(stderr, "[ERROR in OpenCLDriver::HI_memcpy_asyncS()] Cannot find a device pointer (%lx) to memory handle mapping; exit!\n", (unsigned long)src);
#ifdef _OPENARC_PROFILE_
			HI_print_device_address_mapping_entries(tconf->threadID);
    ...
    if( kernelParams->num_args == 0 ) { 
        if( num_args > 0 ) { 
            ...
            kernelParams->kernelParams = (void**)malloc(sizeof(void*) * num_args);
      		...
      		kernelParams->kernelParamSubBuffers = (cl_mem*)malloc(sizeof(cl_mem) * num_args);
        } else { 
...
HI_error_t OpenCLDriver::HI_register_kernel_arg(std::string kernel_name, int arg_index, size_t arg_size, void *arg_value, int arg_type, int arg_trait, size_t unitSize, int threadID)
{
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
	...
	if( arg_type == 0 ) { //scalar variable
    	...
    	*(kernelParams->kernelParamSubBuffers + arg_index) = NULL;
	} else { //pointer variable
		HI_device_mem_handle_t tHandle;
		size_t dataSize = 0;
		if( HI_get_device_mem_handle(*((void **)arg_value), &tHandle, &dataSize, tconf->threadID) == HI_success ) {
			cl_mem localMemHandle = tHandle.memHandle;
			if( tHandle.offset == 0 ) {
				*(kernelParams->kernelParams + arg_index) = (void *)&localMemHandle;
				*(kernelParams->kernelParamsOffset + arg_index) = 0;
				*(kernelParams->kernelParamsInfo + arg_index) = arg_trait;
    			*(kernelParams->kernelParamSubBuffers + arg_index) = NULL;
    			err = clSetKernelArg((cl_kernel)(tconf->kernelsMap.at(this).at(kernel_name)), arg_index, arg_size, &localMemHandle);
			} else {
				size_t localOffset = tHandle.offset;
				*(kernelParams->kernelParams + arg_index) = (void *)&localMemHandle;
				*(kernelParams->kernelParamsOffset + arg_index) = localOffset/unitSize;
				*(kernelParams->kernelParamsInfo + arg_index) = arg_trait;
				cl_buffer_region localRegion;
				localRegion.origin = localOffset;
				localRegion.size = dataSize - localOffset;
				cl_mem_flags flags;
    			...
    			cl_mem subBuffer = clCreateSubBuffer((cl_mem)localMemHandle, flags, CL_BUFFER_CREATE_TYPE_REGION, &localRegion, &err);
    			*(kernelParams->kernelParamSubBuffers + arg_index) = subBuffer;
    			err = clSetKernelArg((cl_kernel)(tconf->kernelsMap.at(this).at(kernel_name)), arg_index, arg_size, &subBuffer);
			}
		} else {
...
HI_error_t OpenCLDriver::HI_kernel_call(std::string kernel_name, size_t gridSize[3], size_t blockSize[3], int async, int num_waits, int *waits, int threadID)
{
    ...
    if(async != (DEFAULT_QUEUE+tconf->asyncID_offset)) {
        cl_event *event = getEvent(async, tconf->threadID);
		//[DEBUG on June 9, 2021] passing kernelParamsOffset is generally incorrect.
        //err = clEnqueueNDRangeKernel(queue, (cl_kernel)(tconf->kernelsMap.at(this).at(kernel_name)), 3, kernelParams->kernelParamsOffset, globalSize, localSize, 0, NULL, event);
		...
		//[DEBUG on June 9, 2021] passing kernelParamsOffset is generally incorrect.
        //err = clEnqueueNDRangeKernel(queue, (cl_kernel)(tconf->kernelsMap.at(this).at(kernel_name)), 3, kernelParams->kernelParamsOffset, globalSize, localSize, 0, NULL, NULL);
	...
	for( int i=0; i<num_args; i++ ) {
		cl_mem subBuffer = kernelParams->kernelParamSubBuffers[i];
		if( subBuffer != NULL ) {
...
#ifdef _OPENARC_PROFILE_
	if( HI_openarcrt_verbosity > 2 ) {
		for( int i=0; i<kernelParams->num_args; i++ ) {
        	fprintf(stderr, "[INFO in OpenCLDriver::HI_kernel_call(%s)] Offeset of argument %d = %lu\n",c_kernel_name, i, kernelParams->kernelParamsOffset[i]);
		}
	}
#endif
    if (err != CL_SUCCESS) {
    	...
    	fprintf(stderr, "\tglobal_work_size: %lu %lu %lu\n", globalSize[2], globalSize[1], globalSize[0]);
    	fprintf(stderr, "\tlocal_work_size: %lu %lu %lu\n", localSize[2], localSize[1], localSize[0]);
		exit(1);
        return HI_error;
    }
#ifdef _OPENARC_PROFILE_
...
#endif
    HostConf_t * tconf = getHostConf(threadID);
	//[FIXME on April 16, 2021] temporarily disabled due to a dealloction error.
...
void OpenCLDriver::HI_malloc(void **devPtr, size_t size, HI_MallocKind_t flags, int threadID) {
	...
	cl_mem memHandle;
	cl_mem_flags mem_flags = convert2CLMemFlags(flags);
    memHandle = clCreateBuffer(clContext, mem_flags, size, NULL, &err);
#ifdef _OPENARC_PROFILE_
	tconf->IDMallocCnt++;
