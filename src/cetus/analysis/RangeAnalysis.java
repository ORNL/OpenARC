...
*/                PrintTools.printlnStatus(toPrettyRanges(symtab, ret, Integer.valueOf(0)), 2);
                return ret;
            }
        }
...
/*        if (debug >= 2) {
        ...
        PrintTools.printlnStatus(toPrettyRanges(proc, ret, Integer.valueOf(0)), 2);
        return ret;
    }
    ...
    */
    public static RangeDomain getRangeDomain(Statement stmt) {
        ...
        cfg.normalize();
        cfg.topologicalSort(cfg.getNodeWith("stmt", "ENTRY"));
        DFANode last = cfg.getNodeWith("top-order", Integer.valueOf(cfg.size()-1));
        DFANode exit = new DFANode("stmt", "EXIT");
        exit.putData("top-order", Integer.valueOf(cfg.size()));
    ...
    */
    ...
    private static void setBackedge(DFANode node) {
        ...
        for (DFANode pred : node.getPreds()) {
            if (my_order < (Integer)pred.getData("top-order")) {
                node.putData("has-backedge", Boolean.valueOf(true));
                break;
            }
        }
    }
    ...
    private static void iterateToFixpoint(CFGraph g, boolean widen) {
        ...
        while (work_list.size() > 0) {
            ...
            if (visits == null) {
                node.putData("num-visits", Integer.valueOf(1));
                setBackedge(node);
            } else {
                node.putData("num-visits", Integer.valueOf(visits + 1));
            }
