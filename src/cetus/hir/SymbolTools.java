    ...
    */
    ...
    public static Symbol getSymbolOf(Expression e) {
        if (e instanceof Identifier) {
        	...
        	//[DEBUG by Seyong Lee on Dec. 3, 2021] extend to be able to handle simple binary expressions if unary operation is about dereferencing.
            //return getSymbolOf(((UnaryExpression)e).getExpression());
            UnaryExpression ue = (UnaryExpression)e;
            if (ue.getOperator() == UnaryOperator.DEREFERENCE) {
            	Expression tExp = ue.getExpression();
            	if( tExp instanceof BinaryExpression ) {
                    DFIterator<Expression> iter =
                            new DFIterator<Expression>(tExp, Expression.class);
                    ...
                    while (iter.hasNext()) {
                        ...
                        if( o instanceof Identifier ) {
                        	Symbol tSym = ((Identifier)o).getSymbol();
                        	if( isPointer(tSym) || isArray(tSym) )  {
                        		return tSym;
                        	}
                        } else if( o instanceof ArrayAccess ) {
                        	if( isPointer(o) ) {
                        		return getSymbolOf(((ArrayAccess)o).getArrayName());
                        	}
                        } else if( o instanceof AccessExpression ) {
                        	Expression memberExp = ((AccessExpression)o).getRHS();
                        	if( memberExp instanceof Identifier ) {
                        		Symbol memSym = ((Identifier)memberExp).getSymbol();
                        		if( isPointer(memSym) || isArray(memSym) ) {
                        			return new AccessSymbol((AccessExpression)o);
                        		}
                        	} else if( isPointer(memberExp) ) {
                        		return new AccessSymbol((AccessExpression)o);
                        	}                        	
                        }
                    }
            	} else {
            		return getSymbolOf(ue.getExpression());
            	}
            } else {
            	return getSymbolOf(ue.getExpression());
            }
