        			...
        			if (token_array[0].compareTo("#") == 0 &&
        				...
        				if (old_annot_key.compareTo("omp") == 0) {
        					...
        					attach_to_next_annotatable =
        							OmpParser.parse_omp_pragma(new_map, token_array, macroMap);
...
/*
        					for( String tString : token_array ) {
        						PrintTools.print(tString + " ", 0);
        					}
        					PrintTools.println("", 0);
*/
        					if ((old_annot_key.compareTo("openarc")==0)) {
        						String old_annot_key2 = token_array[3];
        						if( old_annot_key2.equals("#") ) { //Preprocess macros on OpenACC/OpenARC directives.
        							String [] sub_token_array = Arrays.copyOfRange(token_array, 2, token_array.length);
        							ACCParser.preprocess_acc_pragma(sub_token_array, macroMap);
        							//continue;
        						} 
        					}
    ...
    private String modifyAnnotationString(String old_annotation_str) {
        ...
        if( old_annotation_str.startsWith("/*") || old_annotation_str.startsWith("//") ) {
            ...
            //old_annotation_str = old_annotation_str.replace("{", " { ");
            //old_annotation_str = old_annotation_str.replace("}", " } ");
            old_annotation_str = old_annotation_str.replace("#define", " # define");
        }
