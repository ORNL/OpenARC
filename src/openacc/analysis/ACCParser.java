	    ...
	    public static Expression parse (final String new_expr)
	    {
	    	...
	    	if( containDigits && !containLetters && !containNonAlphaNumeric ) {
	    		if( containDot ) {
	    			...
	    			Double dVal = Double.valueOf(expr);
...
/*			System.out.println("\nmacroMap\n");
	...
	public static boolean parse_acc_pragma(HashMap input_map, String [] str_array, HashMap<String, String>macro_map)
	{
		...
		try {
			...
			case acc_serial                 : parse_acc_serial(); return true;
	 ...
	 *      pipeout( list )
	 * --------------------------------------------------------------- */
	private static void parse_acc_serial()
	{
		addToMap("serial", "_directive");
		if (check("loop")) {
			eat();
			parse_acc_serial_loop();
		} else {
			PrintTools.println("ACCParser is parsing [serial] directive", 3);
			while (end_of_token() == false) 
			{
				String tok = get_token();
				if( tok.equals("") ) continue; //Skip empty string, which may occur due to macro.
	...
	/** ---------------------------------------------------------------
	 *		OpenACC seiral loop Construct
	 *
	 *		#pragma acc serial loop [clause[[,] clause]...] new-line
	 *			structured-block
	 *
	 *		where clause is one of the following
	 *      if( condition )
	 ...
	 * 		independent
	 * --------------------------------------------------------------- */
	private static void parse_acc_serial_loop()
	{
		PrintTools.println("ACCParser is parsing [serial loop] directive", 3);
		addToMap("loop", "_directive");
		while (end_of_token() == false) 
		{
			String tok = get_token();
	...
	private static void parse_arc_transform()
	{
		...
		while (end_of_token() == false) 
		{
			...
			try {
				...
				case token_novectorize	:	parse_acc_noargclause(tok); break;
				case token_vectorfriendly	:	parse_acc_noargclause(tok); break;
