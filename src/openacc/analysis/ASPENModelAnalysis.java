				...
				while(tokenizer.hasMoreTokens()) {
					...
					if( eqIndex != -1) {
						...
						if(opt.equals(FUNCTIONS)) {
							...
							try {
								int value = Integer.valueOf(option.substring(eqIndex+1).trim()).intValue();
			...
			if( !lExpressionAnalyzer.LOADS.isEmpty() ) {
				...
				if( !symStatusMap.isEmpty() ) {
					...
					for( Symbol lSym : symStatusMap.keySet() ) {
						...
						ASPENResource existingLoadRSC = null;
						for( ASPENResource lRSC : loadsSet ) {
							if((lRSC.getID() != null) && (lRSC.getID().toString().equals(lSym.getSymbolName())) ) {
								loadSymExists = true;
								existingLoadRSC = lRSC;
								break;
							}
						}
						//System.err.println("Current load symbol: " + lSym + " : existing? " + loadSymExists);
						//[DEBUG on Jan. 14, 2021] If load resource exists, merge the existing one with analyzed results.
						//if( !loadSymExists ) {
							List specs = lSym.getTypeSpecifiers();
							NameID nParamID = getBuiltInParamForTypes(specs, internalParamMap, mainTrUnt);
							Map<Expression, Expression> statusMap = symStatusMap.get(lSym);
							for(Expression tstatus : statusMap.keySet()) {
								Expression loadSize = null;
								if( loadSymExists ) {
									Expression tLoadSize = existingLoadRSC.getValue();
									if( tLoadSize.toString().equals("0") ) {
										loadSize = new BinaryExpression(statusMap.get(tstatus).clone(), 
												BinaryOperator.MULTIPLY, nParamID.clone());
										existingLoadRSC.setValue(loadSize);
									} else {
										loadSize = tLoadSize;
									}
								} else {
			...
			if( inASPENModelRegion && (lExpressionAnalyzer.allocatedElems == null) ) {
				...
				if( !symStatusMap.isEmpty() ) {
					...
					for( Symbol lSym : symStatusMap.keySet() ) {
						...
						ASPENResource existingStoreRSC = null;
						for( ASPENResource lRSC : storesSet ) {
							if((lRSC.getID() != null) && (lRSC.getID().toString().equals(lSym.getSymbolName())) ) {
								storeSymExists = true;
								existingStoreRSC = lRSC;
								break;
							}
						}
						//[DEBUG on Jan. 14, 2021] If store resource exists, merge the existing one with analyzed results.
						//if( !storeSymExists ) {
							List specs = lSym.getTypeSpecifiers();
							NameID nParamID = getBuiltInParamForTypes(specs, internalParamMap, mainTrUnt);
							Map<Expression, Expression> statusMap = symStatusMap.get(lSym);
							for(Expression tstatus : statusMap.keySet()) {
								Expression storeSize = null;
								if( storeSymExists ) {
									Expression tStoreSize = existingStoreRSC.getValue();
									if( tStoreSize.toString().equals("0") ) {
										storeSize = new BinaryExpression(statusMap.get(tstatus).clone(), 
												BinaryOperator.MULTIPLY, nParamID.clone());
										existingStoreRSC.setValue(storeSize);
									} else {
										storeSize = tStoreSize;
									}
								} else {
									storeSize = new BinaryExpression(statusMap.get(tstatus).clone(), 
											BinaryOperator.MULTIPLY, nParamID.clone());
			...
			ASPENAnnotation annot)
	{
		...
		while(eiter.hasNext())
		{
			...
			if( sym == null ) {
				...
				} else if( !(nid.getParent() instanceof ASPENTrait) && !isInternalParam(newName)  ) {
					//[DEBUG on Jan. 14, 2021] Allow arbitrary trait arguments.
