...
public class AccPrivatization extends AnalysisPass {
	...
	public AccPrivatization(Program program, int option, boolean IRSymOnly) {
		super(program);
		Driver.setOptionValue("privatize", Integer.valueOf(option).toString());
							...
							if( iAnnot != null ) {
								...
								for( Symbol pSym : privatisables ) {
									if( accSharedSymbols.contains(pSym) ) {
										...
										if( sArry == null ) {
											PrintTools.println("[INFO] AccPrivatization found the following symbol can be privatized, " +
													"but it conflicts with user's annotation (included in other shared data clause of an OpenACC loop); "
					...
					if( iAnnot != null ) {
						...
						for( Symbol pSym : privatisables ) {
							if( accSharedSymbols.contains(pSym) ) {
								...
								if( sArry == null ) {
									PrintTools.println("[INFO] AccPrivatization found the following symbol can be privatized, " +
											"but it conflicts with user's annotation (included in other shared data clause of an OpenACC compute region); "
											+ "privatization of this symbol is skipped!\n" +
								...
								if( sArry == null ) {
									PrintTools.println("[INFO] AccPrivatization found the following symbol can be privatized as a firstprivate variable, " +
											"but it conflicts with user's annotation (not included in a firstprivate clause of an OpenACC compute region); "
											+ "privatization of this symbol is skipped!\n" +
											"To enforce privatization, put the symbol in the pcopy clause.\n" +
											"Symbol: " + pSym.getSymbolName() + "\nACCAnnotation: " + cAnnot + "\n", 0);
									continue;
