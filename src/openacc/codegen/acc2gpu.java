...
public class acc2gpu extends CodeGenPass
{
	...
	private boolean convertOpenMPtoOpenMP = false;
	...
	private int privateTransformOption = 0; //0 
	private boolean enableOpenCLArrayFlattening = false;
	...
	protected void cleanAnnotations() {
		////////////////////////////////////////////////////////////////////////////
		// Clean annotations depending on the level of showInternalAnnotations.   //
		// Exception: if convertOpenMP3toOpenACC or convertOpenMP4toOpenACC is on //
		//     and SkipGPUTranslation > 0, at least OpenACC annotations should be //
		//     be included in the output files.                                   //
		////////////////////////////////////////////////////////////////////////////
		if( SkipGPUTranslation > 0 ) {
			if( (convertOpenMP3toOpenACC || convertOpenMP4toOpenACC) || (enableAdvancedMapping > 0) ) {
	...
	public void start()
	{
		List<Statement> tempComputeRegions = new LinkedList<Statement>();
		/////////////////////////////////////////////////////////////////
		...
		if( value != null ) {
			...
			if( defaultNumAsyncQueues > 32 ) {
				PrintTools.println("\n[WARNING] the value of the defaultNumAsyncQueues is bigger than the implementation limit (32); 32 will be used as the value of defaultNumAsyncQueues.", 0);
				defaultNumAsyncQueues = 32;
			}
		}
		...
		value = Driver.getOptionValue("fpgaReduction");
		if( value != null ) {
			fpgaReduction = Integer.valueOf(value).intValue();
		}
...
/*
			...
			if( OPENARC_ARCH == 1 ) {
				...
				//Enable this option when targeting OpenCL on an Apple machine
...
*/			
		...
		value = Driver.getOptionValue("parallelismMappingStrat");
		if( value != null ) {
			if( !value.equals("1") ) { //otions.equals("1") means no suboption is specified.
				...
				while(tokenizer.hasMoreTokens()) {
					...
					if( eqIndex != -1) {
						...
						if(opt.equals("enableAdvancedMapping") || opt.equals("vectorize") || opt.equals("devicetype") || opt.equals("vectorfriendlyanalysis")
								|| opt.equals("privatetransform")) {
							try {
								int arg = Integer.valueOf(option.substring(eqIndex+1).trim()).intValue();
								if(opt.equals("enableAdvancedMapping")) {
									enableAdvancedMapping = arg;
									//[DEBUG on April 8, 2022] Below is disabled so that all the suboptions are parsed regardless of the value of
									//the enableAdvancedMapping option.
									/*
									if( arg == 0 ) {
										break; //all suboptions are ignored.
									}
									*/
								} else if(opt.equals("vectorize")) {
									vectorizationMode = arg;
								} else if(opt.equals("vectorfriendlyanalysis")) {
									vectorFriendlyAnalysis = arg;
								} else if( opt.equals("devicetype")) {
									preferredDeviceType = arg;
								} else if( opt.equals("privatetransform")) {
									privateTransformOption = arg;
								} else {
								PrintTools.println("[ERROR in acc2gpu.start()] " +
										"error in reading parallelismMappingStrat option ("+ option + "); this option will be ignored!", 0);
								break;
								}
							}
...
/*		value = Driver.getOptionValue("tinline");
		...
		if(convertOpenMP4toOpenACC || convertOpenMPtoOpenMP)
		{
		...
		if( !convertOpenACCtoOpenMP3 && !convertOpenACCtoOpenMP4 && !convertOpenMPtoOpenMP &&
				!((convertOpenMP3toOpenACC || convertOpenMP4toOpenACC) && (SkipGPUTranslation > 0)) ) {
        ...
        TransformPass.run(new SerialTransformation(program));
...
/*		if(applyInlining) {
		...
		if( cloneKernelCallingProcedures != 0 ) {
			//[DEBUG on March 28, 2022] KernelCallingProcCloning is disabled if targeting OpenMP3, OpenMP4, or OpenACC,
			//without generating device-specific codes (e.g., CUDA, HIP, or OpenCL).
			if( !convertOpenACCtoOpenMP3 && !convertOpenACCtoOpenMP4 && !convertOpenMPtoOpenMP &&
				!((convertOpenMP3toOpenACC || convertOpenMP4toOpenACC) && (SkipGPUTranslation > 0)) ) {
		...
		if( AccParallelization > 0 ) {
			...
			List<ACCAnnotation> loopAnnots = IRTools.collectPragmas(program, ACCAnnotation.class, "loop");
			if( loopAnnots != null ) {
				for( ACCAnnotation lAnnot : loopAnnots ) {
					if( lAnnot.containsKey("auto") ) {
						PrintTools.println("\n[WARNING] the following loop directive has an auto clause, which will be executed sequentially "
								+ "unless automaic parallelization is enabled. " +
								"To enable automatic parallelization, set AccParallelization to 1.\n" +
								"Loop Directive: " + lAnnot + "\n" + 
		...
		//////////////////////////////////////////////////////////////////////////////////
		TransformPass.run(new ACCLoopDirectivePreprocessor(program, enableAdvancedMapping, vectorizationMode, vectorFriendlyAnalysis, preferredDeviceType));
		...
		//[DEBUG on July 23, 2020] KernelsSpliting pass is move from here to the point after AccReduction pass.
		...
		if( slidingWindowTransformation == 2 ) {
			TransformPass.run(new SlidingWindowTransformation(program, OPENARC_ARCH, tempComputeRegions));
		} else if( slidingWindowTransformation == 1 ) {
			if( (OPENARC_ARCH == 3) || (OPENARC_ARCH == 6) ) {
				TransformPass.run(new SlidingWindowTransformation(program, OPENARC_ARCH, tempComputeRegions));
			}
		}

		if( pipelineTransformation > 0 ) {
			TransformPass.run(new PipeTransformation(program, OPENARC_ARCH, tempComputeRegions));
		}
		...
		//DEBUG: KernelsSplitting transformation pass and annotateUserDirectives passes were moved from here to the above.
		...
		AnalysisPass.run(new CompRegionConfAnalysis(program, OPENARC_ARCH, enableAdvancedMapping));
		...
		if(convertOpenACCtoOpenMP4 || convertOpenMPtoOpenMP) {
			//ACCtoOMP4Translator.ACCtoOMP4Trans(program);
			ACCtoOMP4Translator acc2OMPTranslator = new ACCtoOMP4Translator(program, defaultNumAsyncQueues, enableAdvancedMapping, vectorizationMode, privateTransformOption);
			TransformPass.run(acc2OMPTranslator);
			cleanAnnotations();
			return;
		}
		...
		ACCLoopDirectivePreprocessor.convertVectorToWorkerLoop(program);
		...
		///////////////////////////////////////////////////////
		List<ACCAnnotation> refComputeRegionAnnots = new LinkedList<ACCAnnotation>(); 
		List<Statement> tempCompRegionLocations = new LinkedList<Statement>();
        if( (OPENARC_ARCH <= 0) || (OPENARC_ARCH == 5) )
        {
            PrintTools.println("[ACC2CUDATranslator] begin", 0);
            ACC2CUDATranslator a2cpass = new ACC2CUDATranslator(program, refComputeRegionAnnots, OPENARC_ARCH, defaultNumAsyncQueues);
            a2cpass.start();
        ...
        else
        {
        	if( OPENARC_ARCH == 6 ) {
        		PrintTools.println("[ACC2CUDATranslator] begin", 0);
        		ACC2CUDATranslator a2cupass = new ACC2CUDATranslator(program, refComputeRegionAnnots, 0, defaultNumAsyncQueues);
        		a2cupass.detachTempComputeRegions(tempComputeRegions, tempCompRegionLocations);
        		a2cupass.start();
        		if( enableFaultInjection ) {
        			a2cupass.removeAddressSpaceQualifier();
        		}
        		/*
        		PrintTools.println("[Print refeComputeRegions after removeAddressSpaceQualifier() call]", 0);
        		for( ACCAnnotation tAnnot : refComputeRegionAnnots ) {
        			Statement tStmt = (Statement)tAnnot.getAnnotatable();
        			PrintTools.println(tStmt.toString(), 0);
        		}
        		*/
        		PrintTools.println("[ACC2CUDATranslator] end", 0);
        		//ACC2OPENCLTranslator should be invoked when OPENARC_ARCH == 6.
        		PrintTools.println("[ACC2OPENCLTranslator] begin", 0);
        		ACC2OPENCLTranslator a2clpass = new ACC2OPENCLTranslator(program, refComputeRegionAnnots, 1, defaultNumAsyncQueues);
        		a2clpass.generateDeviceKernels();
        		a2clpass.addStandardHeadersToKernelsTranlationUnit(); 
        		PrintTools.println("[ACC2OPENCLTranslator] end", 0);
        		if( enableOpenCLArrayFlattening ) {
        			PrintTools.println("[OpenCLArrayFlattener] begin", 0);
        			OpenCLArrayFlattener arrayFlattener = new OpenCLArrayFlattener(program, assumeNoAliasingAmongKernelArgs, OPENARC_ARCH, 1);
        			arrayFlattener.start();
        			PrintTools.println("[OpenCLArrayFlattener] end", 0);
        		}
        		PrintTools.println("[OpenCLTranslationTools] begin", 0);
        		OpenCLKernelTranslationTools openclTool = new OpenCLKernelTranslationTools(program, OPENARC_ARCH, 1);
        		openclTool.start();
        		PrintTools.println("[OpenCLTranslationTools] end", 0);
        		PrintTools.println("[ACC2OPENCLTranslator] begin", 0);
        		a2clpass = new ACC2OPENCLTranslator(program, refComputeRegionAnnots, 3, defaultNumAsyncQueues);
        		a2clpass.attachTempComputeRegions(tempComputeRegions, tempCompRegionLocations);
        		a2clpass.generateDeviceKernels();
        		a2clpass.addStandardHeadersToKernelsTranlationUnit(); 
        		PrintTools.println("[ACC2OPENCLTranslator] end", 0);
        		if( enableOpenCLArrayFlattening ) {
        			PrintTools.println("[OpenCLArrayFlattener] begin", 0);
        			OpenCLArrayFlattener arrayFlattener = new OpenCLArrayFlattener(program, assumeNoAliasingAmongKernelArgs, OPENARC_ARCH, 3);
        			arrayFlattener.start();
        			PrintTools.println("[OpenCLArrayFlattener] end", 0);
        		}
        		PrintTools.println("[OpenCLTranslationTools] begin", 0);
        		openclTool = new OpenCLKernelTranslationTools(program, OPENARC_ARCH, 3);
        		openclTool.start();
        		PrintTools.println("[OpenCLTranslationTools] end", 0);
        		a2clpass.removeRefComputeRegions();
        	} else {
        		PrintTools.println("[ACC2OPENCLTranslator] begin", 0);
        		ACC2OPENCLTranslator a2clpass = new ACC2OPENCLTranslator(program, refComputeRegionAnnots, OPENARC_ARCH, defaultNumAsyncQueues);
        		a2clpass.start();
        		PrintTools.println("[ACC2OPENCLTranslator] end", 0);
        		if( enableOpenCLArrayFlattening ) {
        			PrintTools.println("[OpenCLArrayFlattener] begin", 0);
        			OpenCLArrayFlattener arrayFlattener = new OpenCLArrayFlattener(program, assumeNoAliasingAmongKernelArgs, OPENARC_ARCH, OPENARC_ARCH);
        			arrayFlattener.start();
        			PrintTools.println("[OpenCLArrayFlattener] end", 0);
        		}
        		PrintTools.println("[OpenCLTranslationTools] begin", 0);
        		OpenCLKernelTranslationTools openclTool = new OpenCLKernelTranslationTools(program, OPENARC_ARCH, OPENARC_ARCH);
        		openclTool.start();
        		PrintTools.println("[OpenCLTranslationTools] end", 0);
        	}
        }
		...
		/////////////////////////////////////////////////////////////////////////////////////
		// Check whether a CUDA/OpenCL kernel function calls C standard library functions  //
		// that are not supported by CUDA/OpenCL runtime systems.                          //
		// If so, CUDA/OpenCL compiler will fail if they are not inlinable.                //
		/////////////////////////////////////////////////////////////////////////////////////
			...
			if( dot == -1 ) {
				...
				if( dot != -1 ) {
					PrintTools.println("\n[WARNING] Translating a header file , " + iFileName + 
							...
							"which will cause problems if the backend compiler compiles the output codes with different macro definitions or " +
					...
					PrintTools.println("\n[WARNING] Input file name, " + iFileName + 
							...
							"this O2G translation is different from the ones that will be used in the backend compiler.\n", 0);
				}
				continue;
			} else {
				String suffix = iFileName.substring(dot);
				if( !suffix.equalsIgnoreCase(".cu") && !suffix.equalsIgnoreCase(".cl") && !suffix.equalsIgnoreCase(".cpp")) {
		...
		for( String gKey : gKeySet ) {
			...
			for( String option : gOptionSet ) {
				if( excludedGOption.contains(option) ) {
					gOptMap.put(option, new ArrayList<Boolean>(Arrays.asList(Boolean.valueOf(false))));
				} else if( defaultGOption.contains(option) ) {
					gOptMap.put(option, new ArrayList<Boolean>(Arrays.asList(Boolean.valueOf(true))));
				} else {
