...
public class ACC2GPUDriver extends Driver
{
    ...
    optionsWithIntArgument.add("ompaccInter");
		...
		"        =0 does not show any OpenACC/internal annotations (default)\n" +
		"        =1 show only OpenACC annotations\n" +
		...
		optionsWithIntArgument.add("slidingWindowTransformation");

		options.add(options.UTILITY, "fpgaReduction", "N",
		"Control the shift-register based reduction transformation; \n" + 
		"N = 0 (Disable this functionality)\n" +
		"N = 1 (Enable this functionality when targeting single work-item kernels on FPGAs) (default)");
		optionsWithIntArgument.add("fpgaReduction");
		...
		options.add(options.UTILITY, "parallelismMappingStrat",
            "enableAdvancedMapping=number:vectorize=number:vectorfriendlyanalysis=number:devicetype=number:privatetransform=number",
        ...
        + "privatetransform = 0 (add firstprivate/linear clauses to the output OpenMP code (default))\n"
        + "                   1 (do not add firstprivate/linear clauses to the output OpenMP code))"
        + "");
		
		options.add(options.TRANSFORM, "enableOpenCLArrayFlattening", 
		"Enable the OpenCL array flattening transformation.");
	...
	public void setPasses() 
	{
		...
		if( value == null ) {
			setOptionValue("showInternalAnnotations", "0");
		}
		...
		value = getOptionValue("fpgaReduction");
		if( value == null ) {
			setOptionValue("fpgaReduction", "1");
		}
        ...
        if((targetArch != 0) && (targetArch != 6) && (targetArch != -1))
        {
