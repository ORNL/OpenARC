...
/**
  ...
  public static void defaultPrint(KernelFunctionCall call, PrintWriter p)
  {
	...
	for(int i = 0; i < call.getNumArguments(); i++)
	{
		...
		List<Specifier> clonedSpecifierList = new LinkedList<Specifier>();
		for( Specifier tspec : specifierList ) {
			if( !(tspec instanceof OpenCLSpecifier) ) {
				if( !(tspec  instanceof CUDASpecifier) ) {
					clonedSpecifierList.add(tspec);
				} else if( tspec == CUDASpecifier.CUDA_TEXOBJ ) {
					clonedSpecifierList.add(OpenACCSpecifier.OPENACC_TEXOBJ);
				}
			}
		}
...
/*		if( paramDeclarator instanceof NestedDeclarator ) {
		...
		if( targetArch == 4 ) {
			...
			if( isPointer ) {
				...
				p.print(argExp);
				p.print(",");
				p.print(new SizeofExpression(clonedSpecifierList));
				p.print(",");
				if( call.getArgTrait(i).intValue() == trait_readonly ) {
			...
			p.print(","+call.getArgTrait(i));
			//[DEBUG on May 21, 2021] add unit size argument.
			p.print(","+new SizeofExpression(clonedSpecifierList));
		}
		if (targetArch >= 0) {
