...
public abstract class ACC2GPUTranslationTools {
	...
	protected static Set<Symbol> arrayCachingOnRegister(Statement region, Map<Symbol, Set<SubArray>> shrdArryOnRegMap,
		...
		if( shrdArryOnRegMap.isEmpty() ) {
			...
			boolean hasGang = region.containsAnnotation(ACCAnnotation.class, "gang");
			boolean hasWorker = region.containsAnnotation(ACCAnnotation.class, "worker");
			if( hasGang && !hasWorker ) {
				//If the outermost loop of the compute region is gang-only loop, this array-caching-on-register
				//optimization may break the semantics; therefore, do not apply this optimization in this case.
				return arrayElmtCacheSymbols;
			}
		}
...
/*				if( constantIndices ) {
	...
	protected static void seqKernelLoopTransformation(Procedure cProc, ForLoop seqLoop, 
			...
			boolean opt_addSafetyCheckingCode, int targetModel, int targetArch, boolean assumeNoAliasing ) {
		...
		if( (seqLoop.containsAnnotation(ACCAnnotation.class, "private") 
			...
			//[DEBUG on Jan. 22, 2021] firstprivate handling part is disabled, since it is already handled by
			//the preceding privateTransformation pass.
/*
