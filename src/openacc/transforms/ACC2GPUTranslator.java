...
public abstract class ACC2GPUTranslator {
	...
	protected List<ACCAnnotation> referenceComputeRegionAnnots = null;
    ...
    protected int targetModel = 0; //Keep the current translation target.
	...
	protected Integer trait_readonly = Integer.valueOf(0);
	protected Integer trait_writeonly = Integer.valueOf(1);
	protected Integer trait_readwrite = Integer.valueOf(2);
	protected Integer trait_temporary = Integer.valueOf(3);
	...
	protected ACC2GPUTranslator(Program prog, List<ACCAnnotation> refComputeRegionAnnots) {
		program = prog;
		referenceComputeRegionAnnots = refComputeRegionAnnots;
		GPUInitializer();
	}
	...
	public void start() {
		...
		for( Procedure cProc : procedureList ) {
			...
			List<ACCAnnotation> computeRegionAnnots = new LinkedList<ACCAnnotation>();
			...
			if( (dataAnnots != null) && !dataAnnots.isEmpty() ) {
				...
				convComputeRegionsToGPUKernels(cProc, computeRegionAnnots, false);
		...
		if( targetArch == 4 ) {
			...
			for( int i=0; i<main_List.size(); i++ ) {
				...
				initCall.addArgument(new IntegerLiteral(defaultNumAsyncQueues));
		...
		//Remove redundant HI_set_async() function calls in the program.
		for( Procedure cProc : procedureList ) {
			CompoundStatement cStmt = cProc.getBody();
			removeRedundantSetAsyncFunctions(cStmt);
		}
	}
	
	public void generateDeviceKernels() {
		String currentProcName = null;
		Procedure cProc = null;
		List<ACCAnnotation> tCompRegionAnnots = new LinkedList<ACCAnnotation>();
		for( ACCAnnotation tAnnot : referenceComputeRegionAnnots ) {
			Statement tStmt = (Statement)tAnnot.getAnnotatable();
			Statement clonedStmt = tStmt.clone();
			ACCAnnotation clonedAnnot = null;
			CompoundStatement parentCStmt = (CompoundStatement)tStmt.getParent();
			TransformTools.addStatementAfter(parentCStmt, tStmt, clonedStmt);
			Procedure tProc = tStmt.getProcedure();
			String tProcName = tProc.getSymbolName();
			if( tAnnot.containsKey("parallel") ) {
				clonedAnnot = clonedStmt.getAnnotation(ACCAnnotation.class, "parallel");
			} else {
				clonedAnnot = clonedStmt.getAnnotation(ACCAnnotation.class, "kernels");
			}
			if( cProc == null ) {
				currentProcName = tProcName;
				cProc = tProc;
				tCompRegionAnnots.add(clonedAnnot);
			} else if( currentProcName.equals(tProcName) ) {
				tCompRegionAnnots.add(clonedAnnot);
			} else {
				convComputeRegionsToGPUKernels(cProc, tCompRegionAnnots, true);    		
				tCompRegionAnnots.clear();
				currentProcName = tProcName;
				cProc = tProc;
				tCompRegionAnnots.add(clonedAnnot);
			}
		}
		if( (cProc != null) && !tCompRegionAnnots.isEmpty() ) {
			convComputeRegionsToGPUKernels(cProc, tCompRegionAnnots, true);    		
		}
	...
	protected void GPUInitializer() {
		...
		if( value != null ) {
			if( targetArch != 6 ) {
				//Disable MallocPitch when targetArch == 6 to be compatible with OpenCL implementation.
		...
		} else if( targetArch == 6 ) {
			kernelFileNameBase = "kernel";
		}
	...
	protected void handleDeclareDirectives(List<ACCAnnotation> declareAnnots) {
		//int taskRegionCounter = 0;
		Procedure prevProc = null;
		...
		for( ACCAnnotation dAnnot : declareAnnots ) {
			...
			Statement outEnterSubRegionStmt = null;
			Statement inStmt = null;
			CompoundStatement inCStmt = null;
			Statement InExitSubRegionStmt = null;
			...
			if( targetArch == 6 ) {
				 ...
				 */
				outEnterSubRegionStmt = new ExpressionStatement(enterSubRegionCall);
				 ...
				 */
				InExitSubRegionStmt = new ExpressionStatement(exitSubRegionCall);
				outStmt = lastStmts.get(0);
				outCStmt = (CompoundStatement)outStmt.getParent();
				Statement tStmt = AnalysisTools.getStatementBefore(outCStmt, outStmt);
				if( (tStmt != null) && tStmt.toString().equals(InExitSubRegionStmt.toString()) ) {
					outCStmt.removeStatement(tStmt);
					mergeOutSubRegions = true;
				} else {
					outCStmt.addStatementBefore(outStmt, outEnterSubRegionStmt);
				}
				inStmt = firstStmts.get(0);
				inCStmt = (CompoundStatement)inStmt.getParent();
				tStmt = AnalysisTools.getStatementAfter(inCStmt, inStmt);
				if( (tStmt != null) && tStmt.toString().equals(outEnterSubRegionStmt.toString()) ) {
					inCStmt.removeStatement(tStmt);
					mergeInSubRegions = true;
				} else {
					inCStmt.addStatementAfter(inStmt, InExitSubRegionStmt);
				}
			}
			//Current implementation handles IR-symbols only (GPU memory allocation unit is a class object, but not each class member.) 
			handleDataClauses(dAnnot, firstStmts, lastStmts, regionT, IRSymbolOnly, false);
			if( targetArch == 6 ) {
				FunctionCall enterSubRegionCall = new FunctionCall(new NameID("HI_enter_subregion"));
				//enterSubRegionCall.addArgument(new NameID("NULL"));
				//enterSubRegionCall.addArgument(new StringLiteral(cProcName+taskRegionCounter));
				...
				Statement tStmt = AnalysisTools.getStatementAfter(inCStmt, inStmt);
				if( (!mergeInSubRegions) && (tStmt != null) && tStmt.toString().equals(InExitSubRegionStmt.toString()) ) {
					//No statement is added between inStmt and InExitSubRegionStmt; remove InExitSubRegionStmt.
					inCStmt.removeStatement(InExitSubRegionStmt);
				} else {
					...
					for( int i=0; i<lSize; i++ ) {
						...
						if( tt == inStmt ) {
							subRegionStart = true;
						} else if( tt == InExitSubRegionStmt ) {
							...
							if( tt instanceof ExpressionStatement ) {
								...
								if( (tExp instanceof FunctionCall) ) {
									...
									if( tExpName.equals("HI_memcpy") || tExpName.equals("HI_memcpy_async")
											|| tExpName.equals("HI_bind_texref") || tExpName.equals("HI_create_texobj") ) {
					...
					if( taskGeneratingAPIFound ) {
						//Statements are added between inStmt and InExitSubRegionStmt; add InEnterSubRegionStmt.
						inCStmt.addStatementAfter(inStmt, enterSubRegionStmt);
					} else {
						//No statement is added between inStmt and inEnterSubRegionStmt; remove inEnterSubRegionStmt.
						inCStmt.removeStatement(InExitSubRegionStmt);
					}
				}
				tStmt = AnalysisTools.getStatementBefore(outCStmt, outStmt);
				if( (!mergeOutSubRegions) && (tStmt != null) && tStmt.toString().equals(outEnterSubRegionStmt.toString()) ) {
					//No statement is added between outStmt and outEnterSubRegionStmt; remove outEnterSubRegionStmt.
					outCStmt.removeStatement(outEnterSubRegionStmt);
				} else {
					...
					if( taskGeneratingAPIFound ) {
						//Statements are added between outStmt and outEnterSubRegionStmt; add outExitSubRegionStmt.
						outCStmt.addStatementBefore(outStmt, exitSubRegionStmt);
						if( !HIFreeStmts.isEmpty() ) {
							lSize = HIFreeStmts.size();
							for( int i=lSize-1; i>=0; --i ) {
								Statement tFStmt = HIFreeStmts.get(i);
								outCStmt.removeStatement(tFStmt);
								outCStmt.addStatementAfter(exitSubRegionStmt, tFStmt);
							}
						}
					} else {
						//No statement is added between outStmt and outEnterSubRegionStmt; remove outEnterSubRegionStmt.
	...
	protected void handleEnterExitData(Procedure cProc, List<ACCAnnotation> dataRegionAnnots, boolean IRSymOnly) {
		//int taskRegionCounter = 0;
		String cProcName = cProc.getSymbolName();
					...
					if( (dataSet != null) && (!dataSet.isEmpty()) ) {
						...
						for( SubArray sArray : dataSet ) {
							...
							if( lengthList.size() == 0 ) { //hostVar is scalar.
								...
								arg_list.add(new Typecast(castspecs, varName.clone()));
							}
	...
	protected void handleDataRegions(Procedure cProc, List<ACCAnnotation> dataRegionAnnots) {
		...
		for(ACCAnnotation dAnnot : dataRegionAnnots ) {
			...
			if( !(at instanceof Procedure) ) { //Data directive for a procedure is an internal one; skip it.
				...
				//Current implementation handles IR-symbols only (GPU memory allocation unit is a class object, but not each class member.) 
				handleDataClauses(dAnnot, inStmts, outStmts, regionT, IRSymbolOnly, false);
				if( targetArch == 6 ) {
					FunctionCall enterSubRegionCall = new FunctionCall(new NameID("HI_enter_subregion"));
					//enterSubRegionCall.addArgument(new NameID("NULL"));
					//enterSubRegionCall.addArgument(new StringLiteral(cProcName+taskRegionCounter));
...
/*				} else if( fCallName.equals("acc_get_num_devices") || fCallName.equals("acc_set_device_num")
	...
	protected void convComputeRegionsToGPUKernels(Procedure cProc, List<ACCAnnotation> cRegionAnnots, boolean kernelGenOnly) {
		String cProcName = cProc.getSymbolName();
		//int taskRegionCounter = 0;
		...
		for( ACCAnnotation cAnnot : cRegionAnnots ) {
			...
			if( tAnnot.get(kernelType).equals("false") ) {
				continue;
			} else if( (targetArch == 6) && (targetModel == 0) ) {
				List<ARCAnnotation> wAnnots = IRTools.collectPragmas(at, ARCAnnotation.class, "window");
				if( wAnnots.isEmpty() ) {
					Statement clonedStmt = ((Statement)at).clone();
					ACCAnnotation clonedAnnot = clonedStmt.getAnnotation(ACCAnnotation.class, kernelType);
					referenceComputeRegionAnnots.add(clonedAnnot);
					CompoundStatement parentCStmt = (CompoundStatement)at.getParent();
					TransformTools.addStatementAfter(parentCStmt, (Statement)at, clonedStmt);
				}
			}
			if( !kernelGenOnly ) {
		...
		for( ACCAnnotation pAnnot : parallelRegionAnnots ) {
			...
			boolean containsReduction = pAnnot.containsKey("reduction");
			if( !kernelGenOnly ) {
				...
				if(pAnnot.containsKey("async"))
				{
					if( targetArch != 4 ) {
						...
						if(pAnnot.get("async") != null)
						{
							...
							Object asyncObj = pAnnot.get("async");
							if( asyncObj instanceof Expression ) {
								setAsyncCall.addArgument( ((Expression)asyncObj).clone());
							} else {
								setAsyncCall.addArgument(new NameID(asyncObj.toString()));
							}
						}
						else
						{
				...
				if( targetArch != 4 ) {
					...
					//	PrintTools.println("==> Found a compute region with reduction\n" + pAnnot + "\n", 0);
					//	PrintTools.println("\n====> Compute procedure (before data clause handling): \n" + cProc.toString() + "\n", 0);
					//}
					if( targetArch == 6 ) {
						FunctionCall enterSubRegionCall = new FunctionCall(new NameID("HI_enter_subregion"));
						//enterSubRegionCall.addArgument(new NameID("NULL"));
					...
					handleDataClauses(pAnnot, inStmts, outStmts, regionT, IRSymbolOnly, false);
					if( targetArch == 6 ) {
						FunctionCall enterSubRegionCall = new FunctionCall(new NameID("HI_enter_subregion"));
						//enterSubRegionCall.addArgument(new NameID("NULL"));
						//enterSubRegionCall.addArgument(new StringLiteral(cProcName+taskRegionCounter));
					...
					//if( containsReduction ) {
					//	PrintTools.println("\n====> Compute procedure (after data clause handling): \n" + cProc.toString() + "\n", 0);
					//}
				}
			} else {
				List<Statement> inStmts = new LinkedList<Statement>();
				...
				inStmts.add((Statement)at);
				outStmts.add((Statement)at);
				handleDataClauses(pAnnot, inStmts, outStmts, regionT, IRSymbolOnly, true);
			}
			Statement exitSubRegionStmt = null;
			if( (targetArch == 6) && !kernelGenOnly  ) {
				FunctionCall enterSubRegionCall = new FunctionCall(new NameID("HI_enter_subregion"));
				//enterSubRegionCall.addArgument(new NameID("NULL"));
				//enterSubRegionCall.addArgument(new StringLiteral(cProcName+taskRegionCounter));
				enterSubRegionCall.addArgument(new StringLiteral(cProcName));
				 ...
				 * exitSubRegionCall.addArgument(logicalThreadID.clone()); }
				 */
				exitSubRegionStmt = new ExpressionStatement(exitSubRegionCall);
				boolean prefixSubRegionExists = false;
				boolean postfixSubRegionExists = false;
				Statement tStmt = AnalysisTools.getStatementBefore(pComputeStmt, computeStmt);
				if( (tStmt != null) && tStmt.toString().equals(exitSubRegionStmt.toString()) ) {
					prefixSubRegionExists = true;
					pComputeStmt.removeStatement(tStmt);
				} else {
					pComputeStmt.addStatementBefore(computeStmt, enterSubRegionStmt);
				}
				tStmt = AnalysisTools.getStatementAfter(pComputeStmt, computeStmt);
				if( containsReduction ) {
					pComputeStmt.addStatementAfter(computeStmt, exitSubRegionStmt);
				} else {
					if( (tStmt != null) && tStmt.toString().equals(enterSubRegionStmt.toString()) ) {
						postfixSubRegionExists = true;
						pComputeStmt.removeStatement(tStmt);
					} else {
						pComputeStmt.addStatementAfter(computeStmt, exitSubRegionStmt);
					}
			...
			extractComputeRegion(cProc, pAnnot, "parallel", GPUKernelName, IRSymbolOnly, kernelGenOnly);
			if( containsReduction && (targetArch == 6) && !kernelGenOnly  ) {
				List<Traversable> childList = pComputeStmt.getChildren();
				int lSize = childList.size();
				boolean foundExitSubRegionStmt = false;
				Statement HI_kernel_call_stmt = null;
				Statement First_HI_memcpy_stmt = null;
				for( int i=lSize-1; i>=0; i-- ) {
					Traversable tt = childList.get(i);
					if( tt == exitSubRegionStmt ) {
						foundExitSubRegionStmt = true;
					} else if( foundExitSubRegionStmt ) {
						if( tt instanceof ExpressionStatement ) {
							Expression tExp = ((ExpressionStatement)tt).getExpression();
							if( (tExp instanceof KernelFunctionCall)  ) {
								HI_kernel_call_stmt = (Statement)tt;
								break;
							} else if( (tExp instanceof FunctionCall) && ((FunctionCall)tExp).getName().toString().equals("HI_memcpy")) {
								First_HI_memcpy_stmt = (Statement)tt;
							}
						}
					}
				}
				if( HI_kernel_call_stmt != null ) {
					pComputeStmt.removeStatement(exitSubRegionStmt);
					if( First_HI_memcpy_stmt != null ) {
						pComputeStmt.addStatementAfter(First_HI_memcpy_stmt, exitSubRegionStmt);
					} else {
						pComputeStmt.addStatementAfter(HI_kernel_call_stmt, exitSubRegionStmt);
					}
				}
			}
		}
		for( ACCAnnotation kAnnot : kernelsRegionAnnots ) {
			...
			boolean containsReduction = kAnnot.containsKey("reduction");
			...
			if( !kernelGenOnly ) {
				...
				if(kAnnot.containsKey("async"))
				{
					if( targetArch != 4 ) {
						...
						if(kAnnot.get("async") != null)
						{
							...
							Object asyncObj = kAnnot.get("async");
							if( asyncObj instanceof Expression ) {
								setAsyncCall.addArgument( ((Expression)asyncObj).clone());
							} else {
								setAsyncCall.addArgument(new NameID(asyncObj.toString()));
				...
				if( targetArch != 4 ) {
					...
					handleDataClauses(kAnnot, inStmts, outStmts, regionT, IRSymbolOnly, false);
					if( targetArch == 6 ) {
						FunctionCall enterSubRegionCall = new FunctionCall(new NameID("HI_enter_subregion"));
						//enterSubRegionCall.addArgument(new NameID("NULL"));
						//enterSubRegionCall.addArgument(new StringLiteral(cProcName+taskRegionCounter));
				...
				inStmts.add((Statement)at);
				outStmts.add((Statement)at);
				handleDataClauses(kAnnot, inStmts, outStmts, regionT, IRSymbolOnly, true);
			}
			//PrintTools.println("\n====> Compute procedure (after data clause handling): \n" + cProc.toString() + "\n", 0);
			Statement exitSubRegionStmt = null;
			if( (targetArch == 6) && !kernelGenOnly  ) {
			...
			extractComputeRegion(cProc, kAnnot, "kernels", GPUKernelName, IRSymbolOnly, kernelGenOnly);
			if( containsReduction && (targetArch == 6) && !kernelGenOnly  ) {
				List<Traversable> childList = pComputeStmt.getChildren();
				int lSize = childList.size();
				boolean foundExitSubRegionStmt = false;
	...
	protected abstract void handleDataClauses(ACCAnnotation dAnnot, List<Statement> inStmts, List<Statement> outStmts, DataRegionType dReginType,
			boolean IRSymbolOnly, boolean pipeTransformationOnly);
	...
	protected void copyUserSpecifierDeclarations(TranslationUnit tUnit, Traversable tr, Set<Symbol> accessedSymbols,
			Declaration refDecl, String kernelName) {
		...
		PrintTools.println("[INFO from copyUserSpecifierDeclarations()] User symbols accessed in the kernel " + kernelName + ": " + accessedSymbols, 2);
		for( Symbol tSym : accessedSymbols ) {
										...
										for( Declaration mDecl : memberDecls ) {
											if( mDecl instanceof VariableDeclaration ) {
												...
												if( ttypeList != null ) {
													...
													if( !kernelContainsStdintTypes ) {
														if( ttypeList.contains(Specifier.INT8_T) || ttypeList.contains(Specifier.INT16_T) ||
																ttypeList.contains(Specifier.INT32_T) || ttypeList.contains(Specifier.INT64_T) ||
																ttypeList.contains(Specifier.UINT8_T) || ttypeList.contains(Specifier.UINT16_T) ||
																ttypeList.contains(Specifier.UINT32_T) || ttypeList.contains(Specifier.UINT64_T) ) {
															kernelContainsStdintTypes = true;
														}
													}
												}
			...
			if( !declStack.isEmpty() ) {
				while( !declStack.isEmpty() ) {
					...
					if( !addedDeclSet.contains(newDeclStr) ) {
						PrintTools.println("[INFO from copyUserSpecifierDeclarations()] User declarations added to the kernelsTranslationUnit: " + newDeclStr, 2);
...
/*                    if( tProc == null ) {
	...
	public void addStandardHeadersToKernelsTranlationUnit () {
		StringBuilder kernelStr = null;
		boolean openclKernel = true;
		boolean cudaKernel = false;
		boolean genHipKernel = false;
		if( targetArch <= 0 ) {
			cudaKernel = true;
			openclKernel = false;
			...
			String fileName = kernelsTranslationUnit.getOutputFilename();
			int dot = fileName.lastIndexOf(".c");
			if( dot >= 0 ) {
				String suffix = fileName.substring(dot);
				if( !suffix.equalsIgnoreCase(".cl") ) {
					openclKernel = false;
				} 
		...
		//[DEBUG on June 16, 2021] Create offset versions for each CUDA/HIP kernel when targeting the IRIS backend.
		if( (targetArch == 6) && cudaKernel ) {
			List<Procedure> kernelProcList = new LinkedList<Procedure>();
			for(Traversable tChild : kernelsTranslationUnit.getChildren()) {
				if( tChild instanceof Procedure ) {
					Procedure tProc = (Procedure)tChild;
					if( tProc.getReturnType().contains(CUDASpecifier.CUDA_GLOBAL) ) {
						kernelProcList.add(tProc);
					}

				}
			}
			for(Procedure tProc : kernelProcList ) {
				List<Specifier> return_types = tProc.getReturnType();
				CompoundStatement body = (CompoundStatement)tProc.getBody().clone();
				String new_proc_name = tProc.getSymbolName() + "_with_offsets";
				NameID new_procID = new NameID(new_proc_name);
				ProcedureDeclarator c_procDeclr = (ProcedureDeclarator) tProc.getDeclarator();
				...
				VariableDeclarator offset_declarator = new VariableDeclarator(new NameID("blockOff_x"));
				VariableDeclaration offset_decl = new VariableDeclaration(Specifier.SIZE_T, offset_declarator);
				Identifier blockOff_x_ID = new Identifier(offset_declarator);
				newParamList.add(offset_decl);
				offset_declarator = new VariableDeclarator(new NameID("blockOff_y"));
				offset_decl = new VariableDeclaration(Specifier.SIZE_T, offset_declarator);
				Identifier blockOff_y_ID = new Identifier(offset_declarator);
				newParamList.add(offset_decl);
				offset_declarator = new VariableDeclarator(new NameID("blockOff_z"));
				offset_decl = new VariableDeclaration(Specifier.SIZE_T, offset_declarator);
				Identifier blockOff_z_ID = new Identifier(offset_declarator);
				newParamList.add(offset_decl);
				List<Expression> blockIdxList = IRTools.findExpressions(body, new NameID("blockIdx.x"));
				for( Expression tExp : blockIdxList ) {
					BinaryExpression newExp = new BinaryExpression(tExp.clone(), BinaryOperator.ADD, blockOff_x_ID.clone());
					newExp.setParens(true);
					tExp.swapWith(newExp);
				}
				blockIdxList = IRTools.findExpressions(body, new NameID("blockIdx.y"));
				for( Expression tExp : blockIdxList ) {
					BinaryExpression newExp = new BinaryExpression(tExp.clone(), BinaryOperator.ADD, blockOff_y_ID.clone());
					newExp.setParens(true);
					tExp.swapWith(newExp);
				}
				blockIdxList = IRTools.findExpressions(body, new NameID("blockIdx.z"));
				for( Expression tExp : blockIdxList ) {
					BinaryExpression newExp = new BinaryExpression(tExp.clone(), BinaryOperator.ADD, blockOff_z_ID.clone());
					newExp.setParens(true);
					tExp.swapWith(newExp);
				}
				Procedure new_proc = new Procedure(return_types,
						new ProcedureDeclarator(new_procID,
								newParamList), body);	
				kernelsTranslationUnit.addDeclarationAfter(tProc, new_proc);

			}
		}
		if( genHipKernel ) {
			TranslationUnit hipKernelsTranslationUnit = new TranslationUnit(kernelFileNameBase+".hip.cpp");
			StringBuilder newKernelStr = new StringBuilder(64);
			newKernelStr.append("#include <hip/hip_runtime.h>\n");
			CodeAnnotation accHeaderAnnot = new CodeAnnotation(newKernelStr.toString());
			AnnotationDeclaration accHeaderDecl = new AnnotationDeclaration(accHeaderAnnot);
			hipKernelsTranslationUnit.addDeclarationFirst(accHeaderDecl);
			for(Traversable tChild : kernelsTranslationUnit.getChildren()) {
				if( tChild instanceof Declaration ) {
					Declaration childDecl = ((Declaration)tChild).clone();
					hipKernelsTranslationUnit.addDeclaration(childDecl);
				}
			}
			program.addTranslationUnit(hipKernelsTranslationUnit);
		}
	}
	...
	public void removeRefComputeRegions() {
		for( ACCAnnotation tannot : referenceComputeRegionAnnots ) {
			Statement tRegion = (Statement)tannot.getAnnotatable();
			CompoundStatement tParentStmt = (CompoundStatement)tRegion.getParent();
			tParentStmt.removeStatement(tRegion);
		}
	}
	
	public void detachTempComputeRegions(List<Statement> tempComputeRegions, List<Statement>tempComputeRegionLocations) {
		for( Statement tRegion : tempComputeRegions ) {
			//System.out.println("==> Detach the temp compute region: \n" + tRegion);
			Statement dummyStmt = new AnnotationStatement();
			tRegion.swapWith(dummyStmt);
			tempComputeRegionLocations.add(dummyStmt);
		}
	}

	public void attachTempComputeRegions(List<Statement> tempComputeRegions, List<Statement> tempComputeRegionLocations) {
		for( int i=0; i<tempComputeRegionLocations.size(); i++) {
			Statement dummyStmt = tempComputeRegionLocations.get(i);
			Statement tRegion = tempComputeRegions.get(i);
			tRegion.swapWith(dummyStmt);
			ACCAnnotation tAnnot = tRegion.getAnnotation(ACCAnnotation.class, "parallel");
			if( tAnnot == null ) {
				tAnnot = tRegion.getAnnotation(ACCAnnotation.class, "kernels");
			}
			if( tAnnot != null ) {
				//System.out.println("==> Attach the temp compute region: \nAnnotation: " + tAnnot  + "\nCompute region: \n" + tRegion);
				ARCAnnotation arcAnnot1 = tRegion.getAnnotation(ARCAnnotation.class, "ainfo");
				if( arcAnnot1 == null ) {
					Tools.exit("[ERROR in ACC2GPUTranslator.attachTempComputeRegions()] openarc ainfo directive does not exist:\n"
							+ "Annotation: " + tAnnot + AnalysisTools.getEnclosingAnnotationContext(tAnnot));
				} 
				String procname1 = arcAnnot1.get("procname");
				IntegerLiteral kernelID1 = arcAnnot1.get("kernelid");
				boolean foundMatchingKernel = false;
				int m = 0;
				for( m=0; m<referenceComputeRegionAnnots.size(); m++ ) {
					ACCAnnotation refAnnot = referenceComputeRegionAnnots.get(m);
					Annotatable refAt = refAnnot.getAnnotatable();
					ARCAnnotation arcAnnot2 = refAt.getAnnotation(ARCAnnotation.class, "ainfo");
					if( arcAnnot2 == null ) {
						Tools.exit("[ERROR in ACC2GPUTranslator.attachTempComputeRegions()] openarc ainfo directive does not exist:\n"
								+ "Annotation: " + refAnnot + AnalysisTools.getEnclosingAnnotationContext(refAnnot));
					} else {
						String procname2 = arcAnnot2.get("procname");
						IntegerLiteral kernelID2 = arcAnnot2.get("kernelid");
						if( procname2.equals(procname1) && kernelID2.equals(kernelID1) ) {
							foundMatchingKernel = true;
							break;
						}
					}
				}
				if( foundMatchingKernel ) {
					ACCAnnotation refAnnot = referenceComputeRegionAnnots.set(m, tAnnot);
					Statement refStmt = (Statement)refAnnot.getAnnotatable();
					CompoundStatement cStmt = (CompoundStatement)refStmt.getParent();
					cStmt.removeStatement(refStmt);
				} else {
					Tools.exit("[ERROR in ACC2GPUTranslator.attachTempComputeRegions()] cannot find the matching compute region for the following annotation:\n"
							+ "Annotation: " + tAnnot + AnalysisTools.getEnclosingAnnotationContext(tAnnot));
				}
			} else {
				Tools.exit("[ERROR in the ACC2GPUTranslator.attachTempComputeRegions()] cannot find compute directive for the following compute region:\n" + 
						"ACC Compute Region: \n" + tRegion +
						AnalysisTools.getEnclosingContext(tRegion));
			}
		}
	}

	protected void removeRedundantSetAsyncFunctions(CompoundStatement cStmt) {
		List<Statement> removeList = new LinkedList<Statement>();
		Set<IntegerLiteral> asyncSet = new HashSet<IntegerLiteral>();
		List<Traversable> childList = cStmt.getChildren();
		for(Traversable tr : childList) {
			if( tr instanceof ExpressionStatement ) {
				Expression tExp = ((ExpressionStatement)tr).getExpression();
				if( (tExp instanceof FunctionCall) && ((FunctionCall)tExp).getName().toString().equals("HI_set_async") ) {
					//PrintTools.println("Found HI_set_async() function: " + tExp, 0);
					Expression tArg = ((FunctionCall)tExp).getArgument(0);
					if( tArg instanceof IntegerLiteral ) {
						IntegerLiteral tInt = (IntegerLiteral)tArg;
						if( asyncSet.contains(tInt) ) {
							removeList.add((Statement)tr);
						} else {
							asyncSet.add(tInt);
						}
					}
				...
				CompoundStatement tCStmt = (CompoundStatement)tr;
				removeRedundantSetAsyncFunctions(tCStmt);
			} else if( tr instanceof Loop ) {
				CompoundStatement tCStmt = (CompoundStatement)((Loop)tr).getBody();
				removeRedundantSetAsyncFunctions(tCStmt);
			} else if( tr instanceof IfStatement ) {
				CompoundStatement tCStmt = (CompoundStatement)((IfStatement)tr).getThenStatement();
				if( tCStmt != null ) {
					removeRedundantSetAsyncFunctions(tCStmt);
				}
				tCStmt = (CompoundStatement)((IfStatement)tr).getElseStatement();
				if( tCStmt != null ) {
					removeRedundantSetAsyncFunctions(tCStmt);
				}
			}
		...
		if(!removeList.isEmpty()) {
			for(Statement tStmt : removeList) {
				//PrintTools.println("Remove " + tStmt, 0);
				cStmt.removeStatement(tStmt);
			}
		}
	}
}
