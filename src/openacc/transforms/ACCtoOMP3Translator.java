...
/*      // Hide Cetus annotations
      ...
      //[DEBUG on April 17, 2021] simply remove OpenACC API calls when targeting the host CPU, which may break semantics.
      // Replace or OpenACC API calls
      funcCallList = IRTools.getFunctionCalls(program);
      List<Statement> removeStmtList = new LinkedList<Statement>();
      for( FunctionCall fCall : funcCallList ) {
        ...
        if (fName.equals("acc_get_num_devices")) {
        	pStmt = fCall.getStatement();
        	if( pStmt != null ) {
        		removeStmtList.add(pStmt);
        	}
        } 
        ...
        if (fName.equals("acc_copyin") || fName.equals("acc_copyin_async") ||
        		fName.equals("acc_pcopyin") || fName.equals("acc_present_or_copyin") ||
        		fName.equals("acc_create") || fName.equals("acc_create_async") ||
        		fName.equals("acc_pcreate") || fName.equals("acc_present_or_create") ||
        		fName.equals("acc_copyout") || fName.equals("acc_copyout_async") ||
        		fName.equals("acc_delete") || fName.equals("acc_delete_async") ||
        		fName.equals("acc_update_device") || fName.equals("acc_update_device_async") ||
        		fName.equals("acc_update_self") || fName.equals("acc_update_self_async") ||
        		fName.equals("acc_memcpy_to_device") || fName.equals("acc_memcpy_from_device")) {
        	pStmt = fCall.getStatement();
        	if( pStmt != null ) {
        		removeStmtList.add(pStmt);
        	}
      ...
      for( Statement tStmt : removeStmtList ) {
    	  CompoundStatement cStmt = (CompoundStatement)tStmt.getParent();
  ...
  private void parse_parallel_clauses(ACCAnnotation pragma, OmpAnnotation newAnnot, AccRegion regionType) 
  {
    ...
    while (true) {
      ...
      if( at.getParent() instanceof TranslationUnit )  break;
      else {
    	  at = (Annotatable) at.getParent();
      }
      if( at instanceof Procedure )  {
    	  Procedure tProc = (Procedure)at;
    	  if( fCallList == null ) {
    		  Traversable tt = at.getParent();
    		  while( tt != null ) {
    			  if( tt instanceof Program ) {
    				  break;
    			  }
    			  tt = tt.getParent();
    		  }
    		  if( tt instanceof Program ) {
    			  fCallList = IRTools.getFunctionCalls(tt);
    		  }
    	  }
    	  if( fCallList != null ) {
    		  for( FunctionCall fCall : fCallList ) {
    			  if( fCall.getName().equals(tProc.getName()) ) {
    				  at  = (Annotatable)fCall.getStatement(); 
    				  break;
    			  }
    		  }
    	  }
