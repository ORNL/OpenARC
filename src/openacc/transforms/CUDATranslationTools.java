...
public abstract class CUDATranslationTools {
	...
	private static Integer trait_readonly = Integer.valueOf(0);
	private static Integer trait_writeonly = Integer.valueOf(1);
	private static Integer trait_readwrite = Integer.valueOf(2);
	private static Integer trait_temporary = Integer.valueOf(3);
			...
			if( waitStmt == null ) {
				...
				if( confRefStmt == region ) {
					...
					if( tt == null ) { //confRefStmt does not include waitStmt.
						...
						//will not be freed in a timely manner, incurring memory leakage problems.
				...
				if( isWorkerReduction ) {
					...
					if( workerRedCachingOnShared ) {
                        ...
                        if( asyncID == null ) {
                        	...
                        	malloc_call = new FunctionCall(new NameID("HI_tempMalloc1D_async"));
                        }
						...
						if( insertWMalloc ) {
							if( asyncID == null ) {
								...
								Symbol gpu_wred_sym = AnalysisTools.findsSymbol( regionParent.getSymbols(), gpuWRedSymName);
								if(gpu_wred_sym == null)
								{
									regionParent.addDeclaration(gpu_wred_decl.clone());
								}
								regionParent.addStatementBefore(region, gpuBytes_stmt);
								regionParent.addStatementBefore(region, malloc_stmt);
								if( opt_addSafetyCheckingCode ) {
									regionParent.addStatementBefore(region, gMemAdd_stmt.clone());
								}
							}
							/*
...
*/						// Insert "cudaFree(ggred__x);"
						...
						if( asyncID != null ) {
							//cudaFree_call.addArgument(asyncID.clone());
							cudaFree_call.addArgument(new NameID("acc_async_sync"));
						}
						ExpressionStatement cudaFree_stmt = new ExpressionStatement(cudaFree_call);
						//postscriptStmts.addStatement(gpuBytes_stmt.clone());
						refSt = cudaFree_stmt;
				...
				if( insertGMalloc ) {
					...
					if( asyncID == null ) {
						...
						tempMalloc_call = new FunctionCall(new NameID("HI_tempMalloc1D_async"));
					}
					...
					if( asyncID == null ) {
						...
						regionParent.addStatementBefore(region, eMallocStmt);
					}
/*					/////////////////////////////////////////////////////////////////////////
					...
					if( kernelVerification ) {
						...
						if( asyncID == null ) {
							...
							regionParent.addStatementBefore(region, orgGpuBytes_stmt.clone());
							tempMalloc_call = new FunctionCall(new NameID("HI_tempMalloc1D_async"));
						}
					...
					if( asyncID != null ) {
						...
						free_call.addArgument(new NameID("acc_async_sync"));
					}
...
/*					if( asyncID != null ) {
	...
	protected static VariableDeclarator scalarSharedConv(Symbol sharedSym, String symNameBase, List<Specifier> typeSpecs,
			Symbol gpuSym, Statement region, Procedure new_proc, KernelFunctionCall call_to_new_proc, boolean useRegister, 
		...
		/*
		 * if( ROData ) { call_to_new_proc.addArgTrait(trait_readonly); } else {
	...
	protected static VariableDeclarator scalarGangPrivConv(Symbol privSym, String symName, List<Specifier> typeSpecs,
			...
			KernelFunctionCall call_to_new_proc, Expression ggpriv_var) {
	...
	protected static VariableDeclarator arrayPrivConv(Symbol privSym, String symName, List<Specifier> typeSpecs,
			...
			CompoundStatement scope, int privType, KernelFunctionCall call_to_new_proc, Expression ggpriv_var) {
		// Create an extended array type
...
*/			
	...
	protected static void privateTransformation(Procedure cProc, Statement region, String cRegionKind, Expression ifCond,
		...
		//PrintTools.println("Loop Index Symbols = " + loopIndexSymbols, 0);
		//PrintTools.println("Worksharing Loop Index Symbols = " + worksharingLoopIndexSymbols, 0);
...
/*			if( region instanceof CompoundStatement ) {
		...
		if( private_annots != null ) {
			for ( ACCAnnotation pannot : private_annots ) {
				if( pannot.containsKey("worker") ) {
                	...
                	workerpriv_regions.add(pannot);
				} else if( pannot.containsKey("seq") || !pannot.containsKey("gang") ) {
                	//[DEBUG on Dec. 29, 2020] This section should be updated but the current implementation is still OK.
					Annotatable at = pannot.getAnnotatable();
		...
		for ( ACCAnnotation pannot : priv_regions ) {
			...
			if( at.containsAnnotation(ACCAnnotation.class, "worker") ) {
                ...
                isWorkerPrivate = true;
			} else if( !at.containsAnnotation(ACCAnnotation.class, "gang") ) {
				//[DEBUG on Dec. 29, 2020] Update how to handle non-gang/worker loops.
				boolean containInnerGangLoops = AnalysisTools.ipContainPragmas(at, ACCAnnotation.class, "gang", null);
				boolean containInnerWorkerLoops = AnalysisTools.ipContainPragmas(at, ACCAnnotation.class, "worker", null);
				boolean containOuterGangLoops = false;
				boolean containOuterWorkerLoops = false;
				if( AnalysisTools.ipFindFirstPragmaInParent(at, ACCAnnotation.class, "gang", null, null) != null) {
					containOuterGangLoops = true;
				}
				if( AnalysisTools.ipFindFirstPragmaInParent(at, ACCAnnotation.class, "worker", null, null) != null) {
					containOuterWorkerLoops = true;
				}
				if( containInnerGangLoops ) {
					//Private variables in the seq kernel loop is considered as worker-private.
					isWorkerPrivate = true;
				} else if( containInnerWorkerLoops ) {
					if( !containOuterGangLoops ) {
						//Private variables in the seq kernel loop is considered as worker-private.
						isWorkerPrivate = true;
					} else {
						isGangPrivate = true;
					}
				} else if( containOuterWorkerLoops ) {
						isWorkerPrivate = true;
				} else if( containOuterGangLoops ) {
					isGangPrivate = true;
				}
			}
			if( !isSingleTask ) {
			...
			if( isWorkerPrivate ) {//A worker private variable is declared in the innermost worker loop body.
				if( isSingleTask ) {
					...
					ACCAnnotation tAnnot = AnalysisTools.findInnermostPragma(at, ACCAnnotation.class, "worker");
            		if( tAnnot == null ) {
            			tAnnot = AnalysisTools.findInnermostPragma(at, ACCAnnotation.class, "seq");
            		}
            		if( tAnnot == null ) {
            			tAnnot = AnalysisTools.findInnermostPragma(at, ACCAnnotation.class, "vector");
            		}
					ForLoop wLoop = (ForLoop)tAnnot.getAnnotatable();
					scope = (CompoundStatement)(wLoop).getBody();
				}
			...
			//PrintTools.println("Annotatin with private clause: " + pannot, 0);
			//PrintTools.println("First-private symbols: " + FirstPrivSymSet, 0);
...
/*				SymbolTable targetSymbolTable = AnalysisTools.getIRSymbolScope(privSym, region);
				...
				//[DEBUG on June 16, 2020]
				//Work-sharing loop index variable is treated as if thread-private variable.
				//Other loop-index variables will become private depending the sharing scope.
				if( loopIndexSymbols.contains(privSym) ) {
					if( worksharingLoopIndexSymbols.contains(privSym) ) {
                ...
                //[DEBUG on June 12, 2020] isGangPrivate checking is removed since firstprivate is allowed
                //only for parallel region, which is gang-shared.
				...
				if( (FirstPrivSymSet != null) && FirstPrivSymSet.contains(privSym) &&
					...
					continue;
					//[DEBUG on June 16, 2020] worksharing-loop index variable is treated as worker-private.
				}else if( isWorkerPrivate || worksharingLoopIndexSymbols.contains(privSym)) {
					...
					if( !isArray && !isPointer ) { //scalar variable
						...
						if( isWorkerPrivate ) {
							if( at == region ) {
								if( at instanceof ForLoop ) {
                                	...
                                	//[DEBUG on Aug. 20, 2020] It is OK to have a worker-level firstprivate variable for a compound statement
                                	//if the compute region is a single task.
									if( isSingleTask ) {
										Statement last_decl_stmt;
										last_decl_stmt = IRTools.getLastDeclarationStatement(scope);
										if( last_decl_stmt != null ) {
						...
						if( insertMalloc ) {
							...
							if( asyncID == null ) {
								...
								regionParent.addStatementAfter(region, cudaFree_stmt);
								regionParent.addStatementAfter(region, orgGpuBytes_stmt.clone());
							}
	...
	protected static void textureConv(Symbol sharedSym, Identifier textureRefID, Statement region, Expression offset ) {
		Specifier templateType = Specifier.INT;
		if( sharedSym instanceof AccessSymbol ) {
			Symbol memberSym = ((AccessSymbol)sharedSym).getMemberSymbol();
			if( memberSym.getTypeSpecifiers().contains(Specifier.FLOAT)) {
				templateType = Specifier.FLOAT;
			} else if( memberSym.getTypeSpecifiers().contains(Specifier.DOUBLE)) {
				templateType = Specifier.DOUBLE;
			}
			List<Expression> matches = new ArrayList<Expression>(4);
			DFIterator<Expression> iter =
					new DFIterator<Expression>(region, Expression.class);
				...
				if (parent instanceof AccessExpression &&
					...
					if( tExp instanceof ArrayAccess ) {
						...
						if( aAccess.getNumIndices() == 1 ) {
							...
							//[DEBUG on June 22, 2021] tex1Dfetch() is replaced with template function call.
							//FunctionCall texAccessCall = new FunctionCall(new NameID("tex1Dfetch"));
							TemplateFunctionCall texAccessCall = new TemplateFunctionCall(new NameID("tex1Dfetch"));
							texAccessCall.addConfArgument(templateType);
			...
			if( sharedSym.getTypeSpecifiers().contains(Specifier.FLOAT)) {
				templateType = Specifier.FLOAT;
			} else if( sharedSym.getTypeSpecifiers().contains(Specifier.DOUBLE)) {
				templateType = Specifier.DOUBLE;
			}
	...
	protected static void worksharingLoopTransformation(Procedure cProc, CompoundStatement kernelRegion, 
			...
			int defaultNumWorkers, boolean opt_skipKernelLoopBoundChecking, boolean isSingleTask, int targetArch) {
...
/*				if( !localSymbols.isEmpty() ) {
				...
				Expression workerSize = null;
				//if( targetArch == 6 ) {
					if( gangdim == 1 ) {
						workerSize = new NameID("blockDim.x");
					} else if( gangdim == 2 ) {
						workerSize = new NameID("blockDim.y");
					} else {
						workerSize = new NameID("blockDim.z");
					}
				//} else {
				//	workerSize = num_workers.clone();
				//}
				...
				if( isGangLoop ) {
					if( isWorkerLoop ) {
						...
						base = new BinaryExpression(tid, BinaryOperator.ADD, 
								new BinaryExpression(bid, BinaryOperator.MULTIPLY, workerSize));
					} else {
						base = bid;
					}
	...
	/**
	    ...
	    protected static void singleTaskReductionTransformation(Procedure cProc, Statement region, String cRegionKind, Expression ifCond,
	                                                  ...
	                                                  boolean opt_addSafetyCheckingCode, boolean opt_UnrollOnReduction, int maxBlockSize,
	                                                  Expression totalnumgangs, boolean kernelVerification, boolean memtrVerification, FloatLiteral EPSILON,
	                                                  FloatLiteral minCheckValue, int localRedVarConf) {
	        PrintTools.println("[CUDA singleTaskReductionTransformation() begins] current procedure: " + cProc.getSymbolName() +
	                "\ncompute region type: " + cRegionKind + "\n", 2);
	        
	        List<ACCAnnotation> reduction_annots = IRTools.collectPragmas(region, ACCAnnotation.class, "reduction");
	        if( (reduction_annots == null) || reduction_annots.isEmpty() ) {
	        	PrintTools.println("[CUDA singleTaskReductionTransformation() ends early] current procedure: " + cProc.getSymbolName() +
	        			"\ncompute region type: " + cRegionKind + "\n", 2);
	        	return;
	        }
	        ...
	        for ( ACCAnnotation pannot : reduction_regions ) {
	            ...
	            if( scope == null ) {
	                Tools.exit("[ERROR in CUDATranslationTools.singleTaskReductionTransformation() cannot find the scope where reduction" +
	                        "symbols in the following compute region are declared: \n" + region + "\n");
	            }
	            ...
	            for( Symbol redSym : sortedSet ) {
	                ...
	                if( isGangReduction && ((targetSymbolTable == null) || (targetSymbolTable == region)) ) {
	                    ACCAnnotation cAnnot = region.getAnnotation(ACCAnnotation.class, cRegionKind);
	                    Tools.exit("[ERROR in CUDATranslationTools.singleTaskReductionTransformation()] gang-reduction variable should" +
	                            " be visible outside of the enclosing compute region, but the following reduction variable is not; exit!\n" +
	                            "Gang-reduction variable: " + redSym.getSymbolName() + "\n" +
								"Current reduction loop: " + pannot + "\n" +
	                            "Enclosing compute region: " + cAnnot + "\n" +
	                ...
	                if( !foundDimensions ) {
	                    Tools.exit("[ERROR in CUDATranslationTools.singleTaskReductionTransformation()] Dimension information of the following " +
	                            "reduction variable is unknown: " + sArray.getArrayName() + ", Enclosing procedure: " +
	                            cProc.getSymbolName() + "; the ACC2GPU translation failed!");
	                }
	                int dimsize = lengthList.size();
	                ...
	                if( isGangReduction ) {
	                    ...
	                    if( !isArray && !isPointer ) { //scalar variable
	                        ...
	                        if( addSpecs == null ) {
	                        	...
	                        	addSpecs.remove(CUDASpecifier.CUDA_SHARED);
	                        }
	...
	*/						// Insert "cudaFree(ggred__x);"
	                        ...
	                        if( asyncID != null ) {
	                        	...
	                        	HIFree_call.addArgument(new NameID("acc_async_sync"));
	                        }
...
/*	                			if( asyncConfRefChanged ) {
	        ...
	        PrintTools.println("[CUDA singleTaskReductionTransformation() ends] current procedure: " + cProc.getSymbolName() +
	                "\ncompute region type: " + cRegionKind + "\n", 2);
	    }
