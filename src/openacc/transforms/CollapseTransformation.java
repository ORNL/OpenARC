...
public class CollapseTransformation extends TransformPass {
  ...
  public static int collapseLoopHead(ForLoop accLoop, boolean simplifyE) {
    ...
    if( iAnnot != null) {
    	...
    	Set<Symbol> loopIndexSymbols = (Set<Symbol>)iAnnot.get("loopvariables");
		if( loopIndexSymbols == null ) {
			loopIndexSymbols = new HashSet<Symbol>();
			iAnnot.put("loopvariables", loopIndexSymbols);
		}
		loopIndexSymbols.add(newIndex.getSymbol());
    	for(Symbol indexSym : indexSymbols ) {
    		loopIndexSymbols.add(indexSym);
    	}
    }
