...
public abstract class FPGASpecificTools {
  ...
  protected static Statement reductionTransformation(Procedure cProc, Statement region, String cRegionKind,
      KernelFunctionCall call_to_new_proc, Procedure new_proc, boolean IRSymbolOnly, boolean isSingleTask) {

    PrintTools.println("[FPGASpecificTools.reductionTransformation() begins] current procedure: " + cProc.getSymbolName() +
    ...
    boolean STRATIX_5 = false;
    boolean ARRIA_10 = false;
    boolean STRATIX_10 = false;

    String fpga_env = System.getenv("OPENARC_FPGA");
    if (fpga_env == null) {
      PrintTools.println("[WARNING in FPGASpecificTools.reductionTransformation] Environment variable OPENARC_FPGA not set. Options include:\n" + 
          "  STRATIX_5\n  ARRIA_10\n  STRATIX_10\nSTRATIX_10 will be used as a default value, which may degrade the performance", 0);
      STRATIX_10 = true;
    } else if (fpga_env.equals("STRATIX_5"))  {
      STRATIX_5 = true;
    } else if (fpga_env.equals("ARRIA_10")) {
      ARRIA_10 = true;
    } else if (fpga_env.equals("STRATIX_10")) {
      STRATIX_10 = true;
    } else {
    ...
    if (STRATIX_10) {
      ...
      operatorCostMap.put(new OperatorPair(ReductionOperator.ADD, Specifier.FLOAT), 6); 
      operatorCostMap.put(new OperatorPair(ReductionOperator.MULTIPLY, Specifier.FLOAT), 7);
      operatorCostMap.put(new OperatorPair(ReductionOperator.MAX, Specifier.FLOAT), 1);
      operatorCostMap.put(new OperatorPair(ReductionOperator.MIN, Specifier.FLOAT), 1);

      operatorCostMap.put(new OperatorPair(ReductionOperator.ADD, Specifier.DOUBLE), 36);
      operatorCostMap.put(new OperatorPair(ReductionOperator.MULTIPLY, Specifier.DOUBLE), 28);
      operatorCostMap.put(new OperatorPair(ReductionOperator.MAX, Specifier.DOUBLE), 1);
      operatorCostMap.put(new OperatorPair(ReductionOperator.MIN, Specifier.DOUBLE), 1);
    }
    ...
    if( (reduction_annots == null) || reduction_annots.isEmpty() ) {
      PrintTools.println("[FPGASpecificTools.reductionTransformation() ends] current procedure: " + cProc.getSymbolName() +
          "\ncompute region type: " + cRegionKind + "\n", 2);
      return region;
    }
    ...
    for ( ACCAnnotation pannot : reduction_annots) {
        ...
        for( String tKey : keySet ) {
          ...
          if (tKey.equals("unroll"))  {
            if (index == keySet.length - 1)
              Tools.exit("[ERROR in FPGASpecificTools.reductionTransformation: Annotation #pragma unroll must have" + 
      ...
      for (Symbol rSym : redOpMap.keySet()) {
        ...
        if (obj == null)
          Tools.exit("[ERROR in FPGASpecificTools.reductionTransformation] Reduction operator \"" + redOpMap.get(rSym) + "\"  on type \"" 
    ...
    for (ACCAnnotation tOldAnnot : ACCList) {
      ...
      for (String tKey : keySet) {
        ...
        // ignore unneeded clauses
            ...
            tKey.equals("accreadonlyprivate") ||
