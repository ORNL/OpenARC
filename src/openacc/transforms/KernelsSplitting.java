...
public class KernelsSplitting extends TransformPass {
		...
		if( kernelsAnnots != null ) {
			for( ACCAnnotation kAnnot : kernelsAnnots ) {
				...
				if( at instanceof CompoundStatement ) { //Found a kernels region that is not a loop.
					ACCAnnotation intAnnot = at.getAnnotation(ACCAnnotation.class, "internal");
					ARCAnnotation ainfoAnnot = at.getAnnotation(ARCAnnotation.class, "ainfo");
					Set<Symbol> accExplicitSharedSymbols = intAnnot.get("accexplicitshared");
					...
					while( cIndex < kRSize ) {
						...
						if( createKernel > 0 ) {
							...
							if( iAnnot == null ) {
								...
								accExpSharedSymbols = new HashSet<Symbol>();
								...
								accExpSharedSymbols = (Set<Symbol>)iAnnot.get("accexplicitshared");
								if( accExpSharedSymbols == null ) {
									accExpSharedSymbols = new HashSet<Symbol>();
									iAnnot.put("accexplicitshared", accExpSharedSymbols);
								}
							...
							accSharedSymbols.removeAll(accPrivateSymbols);
							for(Symbol tSym : accExplicitSharedSymbols) {
								if( accSharedSymbols.contains(tSym) ) {
									accExpSharedSymbols.add(tSym);
								}
							}
							if( accExpSharedSymbols.isEmpty() ) {
								iAnnot.remove("accexplicitshared");
							}
						}
					}
					...
					if( numNewKernels > 1 ) {
						//We have to relabel each kernel region.
						String procName = cProc.getSymbolName().toString();
						List<ACCAnnotation> compAnnots = AnalysisTools.collectPragmas(cProc, ACCAnnotation.class, ACCAnnotation.computeRegions, false);
						int cnt = 0;
						for( ACCAnnotation cAnnot : compAnnots ) {
							Annotatable ttat = cAnnot.getAnnotatable();
							//String refName = procName + "_kernel" + cnt++;	
							ARCAnnotation aInfo = ttat.getAnnotation(ARCAnnotation.class, "ainfo");
							if( aInfo == null ) {
	...
	public void updateDataClauses() {
		for( ACCAnnotation aAnnot : DataRegions ) {
			...
			//[DEBUG on July 23, 2020] Add a pass to remove private variables from the shared data clauses.
			ACCAnnotation cAnnot = cRegion.getAnnotation(ACCAnnotation.class, "accprivate");
			if( cAnnot != null ) {
				Set<Symbol> accprivateSet = cAnnot.get("accprivate");
				ACCAnnotation DRIAnnot = at.getAnnotation(ACCAnnotation.class, "accshared");
				if( DRIAnnot != null ) {
					Set<Symbol> DataACCsharedSet = DRIAnnot.get("accshared");
					DataACCsharedSet.removeAll(accprivateSet);
					for( String dataClause : ACCAnnotation.dataClauses ) {
						Set<SubArray> dSet = (Set<SubArray>)aAnnot.get(dataClause);
						Set<SubArray> removeSet = new HashSet<SubArray>();
						if( dSet != null ) {
							for( SubArray subArr : dSet ) {
								Symbol sym = AnalysisTools.subarrayToSymbol(subArr, IRSymbolOnly);
								if( accprivateSet.contains(sym) ) {
									removeSet.add(subArr);
								}
							}
							if( !removeSet.isEmpty() ) {
								dSet.removeAll(removeSet);
							}
							if( dSet.isEmpty() ) {
								aAnnot.remove(dataClause);
							}
						}
					}
				}
			}
			cAnnot = cRegion.getAnnotation(ACCAnnotation.class, "accreadonly");
			if( cAnnot == null ) {
				continue;
			}
			...
			//[DEBUG on June 15, 2020] Moved below statements from bottom to here.
			ACCAnnotation tempAnnot = null;
			...
			//PrintTools.println("==> Current data region: "+ at, 0);
			...
			tempAnnot = at.getAnnotation(ACCAnnotation.class, "pcopy");
			if( tempAnnot != null ) {
				compAnnot = tempAnnot;
