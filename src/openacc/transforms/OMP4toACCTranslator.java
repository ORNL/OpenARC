...
import openacc.hir.CUDALibrary;
import openacc.hir.NVLAnnotation;
import openacc.hir.OpenCLLibrary;
...
public class OMP4toACCTranslator extends TransformPass {
	...
	private List<Procedure>  visitedProcList = new LinkedList<Procedure>();
    ...
    public void start() 
    {
    	...
    	for( Traversable trUnt : program.getChildren() ) {
    		...
    		for( OmpAnnotation oAnnot : TargetRegionAnnots ) {
    			...
    			String wType = parse_target_region(targetRegion, taskMappingType, false);
    		}
...
/*    					List<OmpAnnotation> ompAnnotList = at.getAnnotations(OmpAnnotation.class);
    	...
    	HashMap<ProcedureDeclarator, VariableDeclaration>  devProcDeclMap = new HashMap<ProcedureDeclarator, VariableDeclaration>();
    	HashMap<ProcedureDeclarator, Procedure>  devProcMap = new HashMap<ProcedureDeclarator, Procedure>();
    	for( ProcedureDeclarator tProcDeclr : procDeclrList ) {
    		Declaration tProcDecl = tProcDeclr.getDeclaration();
    		ACCAnnotation rAnnot = tProcDecl.getAnnotation(ACCAnnotation.class, "routine");
    		if( rAnnot != null ) {
    			if( tProcDecl instanceof VariableDeclaration ) {
    				devProcDeclMap.put(tProcDeclr, (VariableDeclaration)tProcDecl);
    			} else if( tProcDecl instanceof Procedure ) {
    				devProcMap.put(tProcDeclr, (Procedure)tProcDecl);
    			}
    		}
    	}
    	for( ProcedureDeclarator tProcDeclr : devProcDeclMap.keySet() ) {
    		VariableDeclaration tProcDecl = devProcDeclMap.get(tProcDeclr);
    		ACCAnnotation rAnnot = tProcDecl.getAnnotation(ACCAnnotation.class, "routine");
    		Procedure tProc = devProcMap.get(tProcDecl);
    		if( tProc != null ) {
    			ACCAnnotation rPAnnot = tProc.getAnnotation(ACCAnnotation.class, "routine");
    			if( rPAnnot != null ) {
    				if( rPAnnot.containsKey("gang")) {
    					rAnnot.put("gang", "_clause");
    				} else if( rPAnnot.containsKey("worker")) {
    					rAnnot.put("worker", "_clause");
    				} else if( rPAnnot.containsKey("vector")) {
    					rAnnot.put("vector", "_clause");
    				} else {
    ...
    private void parse_target_pragma(OmpAnnotation pragma, OmpRegion regionType)
    {
        ...
        Set<SubArray> pcreateSet = new HashSet<SubArray>();
        Set<SubArray> copyinSet = new HashSet<SubArray>();
        Set<SubArray> pcopyinSet = new HashSet<SubArray>();
        Set<SubArray> pncopyinSet = new HashSet<SubArray>();
        Set<SubArray> copyoutSet = new HashSet<SubArray>();
        Set<SubArray> pcopyoutSet = new HashSet<SubArray>();
        Set<SubArray> pncopyoutSet = new HashSet<SubArray>();
        Set<SubArray> copySet = new HashSet<SubArray>();
        Set<SubArray> pcopySet = new HashSet<SubArray>();
        Set<SubArray> pncopySet = new HashSet<SubArray>();
        ...
        if(pragma.containsKey("alloc")) {
            parseSet((Set<String>)pragma.get("alloc"), pcreateSet);
            createAccDirective = true;
        }

        if(pragma.containsKey("always alloc")) {
            parseSet((Set<String>)pragma.get("always alloc"), pcreateSet);
            createAccDirective = true;
            always = true;
        }

        if(pragma.containsKey("present alloc")) {
            parseSet((Set<String>)pragma.get("present alloc"), presentSet);
            createAccDirective = true;
            present = true;
        }

        if(pragma.containsKey("always present alloc")) {
            parseSet((Set<String>)pragma.get("always present alloc"), presentSet);
            createAccDirective = true;
            always = true;
            present = true;
        }
        ...
        if(pragma.containsKey("to")) {
            parseSet((Set<String>)pragma.get("to"), pcopyinSet);
            createAccDirective = true;
        }
        ...
        if(pragma.containsKey("present to")) {
            parseSet((Set<String>)pragma.get("present to"), presentSet);
            createAccDirective = true;
            present = true;
        }

        if(pragma.containsKey("always present to")) {
            parseSet((Set<String>)pragma.get("always present to"), pncopyinSet);
            createAccDirective = true;
            always = true;
            present = true;
        }
        ...
        if(pragma.containsKey("from")) {
            parseSet((Set<String>)pragma.get("from"), pcopyoutSet);
            createAccDirective = true;
        }
        ...
        if(pragma.containsKey("present from")) {
            parseSet((Set<String>)pragma.get("present from"), presentSet);
            createAccDirective = true;
            present = true;
        }

        if(pragma.containsKey("always present from")) {
            parseSet((Set<String>)pragma.get("always present from"), pncopyoutSet);
            createAccDirective = true;
            always = true;
            present = true;
        }
        ...
        {
            parseSet((Set<String>)pragma.get("tofrom"), pcopySet);
            createAccDirective = true;
        }
        ...
        if(pragma.containsKey("present tofrom")) 
        {
            parseSet((Set<String>)pragma.get("present tofrom"), presentSet);
            createAccDirective = true;
            present = true;
        }

        if(pragma.containsKey("always present tofrom")) 
        {
            parseSet((Set<String>)pragma.get("always present tofrom"), pncopySet);
            createAccDirective = true;
            always = true;
            present = true;
        }
        ...
        if(pragma.containsKey("use_device_addr"))
        {
            parseSet((Set<String>)pragma.get("use_device_addr"), presentSet);
            createAccDirective = true;
        }
...
/*        		newAnnot.put("kernels", "_directive");
 ...
 /*       		List<OmpAnnotation> delList = IRTools.collectPragmas(at, OmpAnnotation.class, null);
        ...
        if(pcreateSet.size() > 0) {
        	if( (regionType == OmpRegion.Target_Data)  || (regionType == OmpRegion.Target) 
        			|| (regionType == OmpRegion.Target_Enter_Data)) {
        		newAnnot.put("pcreate", pcreateSet);
        	}
        }

        //[FIXME] treat "always present" map-type-modifier as if "always" map-type-modifier, which is incorrect. 
        if(pncopyinSet.size() > 0) {
        	copyinSet.addAll(pncopyinSet);
        }
        if(pncopyoutSet.size() > 0) {
        	copyoutSet.addAll(pncopyoutSet);
        }
        if(pncopySet.size() > 0) {
        	copySet.addAll(pncopySet);
        }
        ...
        if(pcopyinSet.size() > 0) {
        	if( regionType == OmpRegion.Target_Update ) {
        		//[FIXME] acc update directive does not support pcopyin feature.
        		newAnnot.put("device", pcopyinSet);
        	} else if( regionType == OmpRegion.Target_Exit_Data ) {
        		//newAnnot.put("delete", pcopyinSet);
        	} else {
        		newAnnot.put("pcopyin", pcopyinSet);
        	}
        }
        if(copyoutSet.size() > 0) {
        	if( regionType == OmpRegion.Target_Update ) {
        		...
        		//newAnnot.put("create", copyoutSet);
        	} else {
        		newAnnot.put("copyout", copyoutSet);
        	}
        }
        if(pcopyoutSet.size() > 0) {
        	if( regionType == OmpRegion.Target_Update ) {
        		//[FIXME] acc update directive does not support pcopyout feature.
        		newAnnot.put("host", pcopyoutSet);
        	} else if( regionType == OmpRegion.Target_Enter_Data ) {
        		//newAnnot.put("pcreate", pcopyoutSet);
        	} else {
        		newAnnot.put("pcopyout", pcopyoutSet);
        	}
        }
        if(copySet.size() > 0) {
        	if( (regionType == OmpRegion.Target_Data)  || (regionType == OmpRegion.Target) ) {
        ...
        if(deleteSet.size() > 0) {
        	if( regionType == OmpRegion.Target_Exit_Data ) {
        ...
        if( nowait ) {
        	if( createAccDirective ) {
        		//CompoundStatement tCStmt = (CompoundStatement)at.getParent();
        		//PrintTools.println("[Statement before]\n" + tCStmt, 0);
        ...
        if( !deviceFuncList.isEmpty() ) {
        	for( Declaration tDecl : deviceFuncList ) {
        		if( !tDecl.containsAnnotation(ACCAnnotation.class, "routine") ) {
        			...
        			if( tDecl instanceof Procedure ) {
        				CompoundStatement pBody = ((Procedure)tDecl).getBody();
        			}
        		}
        	}
        }
    		...
    		if( !foundDimensions ) {
    			...
    			if( subA.getArrayDimension() == 0 ) {
    				...
    				for( Expression texp : lengthList ) {
    					Expression startIndex = startList.get(i);
    					endList.add(Symbolic.simplify(new BinaryExpression(startIndex.clone(), BinaryOperator.ADD, Symbolic.simplify(new BinaryExpression(texp.clone(), BinaryOperator.SUBTRACT, new IntegerLiteral(1))))));
    					i++;
    				}
    ...
    private String parse_target_region(Annotatable inRegion, int taskMappingType, boolean inDeviceFunction)  {
    	String worksharingType = "seq";
    	ACCAnnotation routineDirective = null;
    	if( inDeviceFunction ) {
    		Procedure parentProc  = (Procedure)inRegion.getParent();
    		routineDirective = parentProc.getAnnotation(ACCAnnotation.class, "routine");
    	}
    	DFIterator<Annotatable> titer = new DFIterator<Annotatable>(inRegion, Annotatable.class);
    	while (titer.hasNext()) 
    	{
    			...
    			for (int i = 0; i < pragmas.size(); i++) 
    			{
    				...
    				if (pragma.containsKey("distribute")) {
    					if( taskMappingType == 0 ) {
    						...
    						parse_distribute_pragma(pragma);
    						if( inDeviceFunction && (routineDirective != null) ) {
    							routineDirective.put("gang", "_clause" );
    							worksharingType = "gang";
    						}
    					} else {
    						pragma.setSkipPrint(true);
    						removeList.add(pragma);
    					...
    					if( taskMappingType == 0 ) {
    						...
    						parse_parallel_pragma(pragma);
    						if( inDeviceFunction && (routineDirective != null) ) {
    							if( !routineDirective.containsKey("gang") ) {
    								routineDirective.put("worker", "_clause" );
    								worksharingType = "worker";
    							}
    						}
    					} else {
    						pragma.setSkipPrint(true);
    					...
    					if( taskMappingType == 0 ) {
    						...
    						parse_simd_pragma(pragma);
    						if( inDeviceFunction && (routineDirective != null) ) {
    							if( !routineDirective.containsKey("gang") && !routineDirective.containsKey("worker") ) {
    								routineDirective.put("vector", "_clause" );
    								worksharingType = "vector";
    							}
    						}
    					} else {
    						pragma.setSkipPrint(true);
    		...
    		if( at instanceof ExpressionStatement ) {
    			List<FunctionCall> tFuncCallList = IRTools.getFunctionCalls(at);
    			if( tFuncCallList != null ) {
    				for( FunctionCall tFuncCall : tFuncCallList ) {
    					if( !StandardLibrary.contains(tFuncCall) && !OpenMPLibrary.contains(tFuncCall) ) { 
    						Procedure tProc = tFuncCall.getProcedure();
    						if( tProc != null )  {
    							if( !visitedProcList.contains(tProc) )  {
    								visitedProcList.add(tProc);
    								String wType = parse_target_region(tProc.getBody(), taskMappingType, true);
    								if( wType.equals("gang") )  {
    									if( routineDirective != null ) {
    										routineDirective.put("gang", "_clause");
    									}
    									worksharingType = "gang";
    								} else if( wType.equals("worker") ) {
    									if( !worksharingType.equals("gang") ) {
    										if( routineDirective != null ) {
    											routineDirective.put("worker", "_clause");
    										}
    										worksharingType = "worker";
    									}
    								} else if( wType.equals("vector") ) {
    									if( !worksharingType.equals("gang") && !worksharingType.equals("worker") ) {
    										if( routineDirective != null ) {
    											routineDirective.put("vector", "_clause");
    										}
    										worksharingType = "vector";
    	...
    	if( inDeviceFunction && (routineDirective != null) ) {
    		if( worksharingType.equals("gang") ) {
    			routineDirective.remove("worker");
    			routineDirective.remove("vector");
    			routineDirective.remove("seq");
    		} else if( worksharingType.equals("worker") ) {
    			routineDirective.remove("gang");
    			routineDirective.remove("vector");
    			routineDirective.remove("seq");
    		} else if( worksharingType.equals("vector") ) {
    			routineDirective.remove("gang");
    			routineDirective.remove("worker");
    			routineDirective.remove("seq");
    		} else {
    			...
    			routineDirective.remove("worker");
    			routineDirective.remove("vector");
    			routineDirective.put("seq", "_clause");
    		}
    	}
    	return worksharingType;
    }
	...
	private void parse_target_pragma_old(OmpAnnotation pragma, OmpRegion regionType)
	{
	    ...
	    if(pragma.containsKey("always alloc")) {
	        parseSet((Set<String>)pragma.get("always alloc"), createSet);
	        always = true;
	    }

	    if(pragma.containsKey("present alloc")) {
	        parseSet((Set<String>)pragma.get("present alloc"), createSet);
	        present = true;
	    }

	    if(pragma.containsKey("always present alloc")) {
	        parseSet((Set<String>)pragma.get("always present alloc"), createSet);
	        always = true;
	        present = true;
	    }
	    ...
	    if(pragma.containsKey("present to")) {
	        parseSet((Set<String>)pragma.get("present to"), copyinSet);
	        present = true;
	    }

	    if(pragma.containsKey("always present to")) {
	        parseSet((Set<String>)pragma.get("always present to"), copyinSet);
	        always = true;
	        present = true;
	    }
	    ...
	    if(pragma.containsKey("present from")) {
	        parseSet((Set<String>)pragma.get("present from"), copyoutSet);
	        present = true;
	    }

	    if(pragma.containsKey("always present from")) {
	        parseSet((Set<String>)pragma.get("always present from"), copyoutSet);
	        always = true;
	        present = true;
	    }
	    ...
	    if(pragma.containsKey("present tofrom")) 
	    {
	        parseSet((Set<String>)pragma.get("present tofrom"), copySet);
	        present = true;
	    }

	    if(pragma.containsKey("always present tofrom")) 
	    {
	        parseSet((Set<String>)pragma.get("always present tofrom"), copySet);
	        always = true;
	        present = true;
	    }
