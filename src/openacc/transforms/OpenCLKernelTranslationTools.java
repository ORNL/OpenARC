...
public class OpenCLKernelTranslationTools extends TransformPass
{
     ...
     * @param program
     */
    public OpenCLKernelTranslationTools(Program program, int targetArch, int targetModel)
    {
        super(program);
		...
		if( targetArch == 6 ) {
			if( targetModel == 3 ) {
				kernelFileNameBase = kernelFileNameBase + "-fpga";
			}
		}
    }
    ...
    public void start()
    {
        ...
        /////////////////////////////////////////////////////////////////////////////////////////
        // Step3: Variable declared in the __local address space inside a kernel function must //
        // occur at kernel function scope. (OpenCL Specification Version 1.2 Section 6.5.2).   //
        // [DEBUG on Sept. 7, 2021] It also checks whether double variables are used or not.   //    
        /////////////////////////////////////////////////////////////////////////////////////////
        boolean doubleUsed = false;
        for(Procedure kernelProc : kernelProcedures)
        {
        	if( !doubleUsed ) {
        		Set<Declaration> paramDecls = kernelProc.getDeclarations();
        		for( Declaration pDecl : paramDecls ) {
        			List pSpecs = ((VariableDeclaration)pDecl).getSpecifiers();
        			if( pSpecs.contains(Specifier.DOUBLE) )  {
        				doubleUsed = true;
        				break;
        			}
        		}
        	...
        	List<VariableDeclaration> localVarDeclList = new LinkedList<VariableDeclaration>();
        	DFIterator<VariableDeclaration> iter =
        			new DFIterator<VariableDeclaration>(kBody, VariableDeclaration.class);
        	iter.pruneOn(Expression.class);
        	while(iter.hasNext()) {
        		VariableDeclaration tVDecl = iter.next();
        		Declarator declr = tVDecl.getDeclarator(0);
        		List vDeclSpec = tVDecl.getSpecifiers();
        		if( !doubleUsed && vDeclSpec.contains(Specifier.DOUBLE) ) {
        			doubleUsed = true;
        		}
        		List<Specifier> declrSpec = declr.getSpecifiers();
        		boolean isLocalVariable = false;
        		boolean isPointer = false;
        		for( Specifier tSpec : declrSpec ) {
        			if( tSpec instanceof PointerSpecifier ) {
        				isPointer = true;
        			} else if( tSpec.equals(OpenCLSpecifier.OPENCL_LOCAL) ) {
        				isLocalVariable = true;
        			}
        			if( isPointer && isLocalVariable ) {
        				localVarDeclList.add(tVDecl);
        				break;
        			}
        		}
        		if( !isLocalVariable ) {
        			if( vDeclSpec.contains(OpenCLSpecifier.OPENCL_LOCAL) ) {
        				localVarDeclList.add(tVDecl);
        			}
        		}
        	}
        	for(VariableDeclaration tVDecl : localVarDeclList) {
        		Traversable tParent = tVDecl.getParent();
        		Traversable tGrandParent = tParent.getParent();
        		if( !tGrandParent.equals(kBody) ) {
        			tGrandParent.removeChild(tParent);
        			tVDecl.setParent(null);
        			Traversable tt = tGrandParent.getParent();
        			while( (tt != null) && (!tt.equals(kBody)) ) {
        				tGrandParent = tt;
        				tt= tGrandParent.getParent();
        			}
        			if( tt.equals(kBody)) {
        				//kBody.addStatementBefore((Statement)tGrandParent, (Statement)tParent);
        				kBody.addDeclaration(tVDecl);
        			}
        		}
        	}
        }
        if( doubleUsed )  {
        	CodeAnnotation accHeaderAnnot = new CodeAnnotation("#pragma OPENCL EXTENSION cl_khr_fp64: enable\n");
        	AnnotationDeclaration accHeaderDecl = new AnnotationDeclaration(accHeaderAnnot);
        	Declaration firstDecl = kernelsTranslationUnit.getFirstDeclaration();
        	kernelsTranslationUnit.addDeclarationAfter(firstDecl, accHeaderDecl);
        }
    }
...
/*    											DFIterator<Identifier> titer =
    										...
    										//[DEBUG on Aug. 17, 2021] changed to check declaration type instead of declarator type.
...
/*    											DFIterator<Identifier> titer =
    										...
    										if( !tSpec.contains(OpenCLSpecifier.OPENCL_CONSTANT) ) {
    										...
    										if( tSpec.contains(OpenCLSpecifier.OPENCL_LOCAL) ||
    												tSpec.contains(OpenCLSpecifier.OPENCL_GLOBAL) ) {
    											Tools.exit("[ERROR in OpenCLKernelTranslationTools.addAddressSpaceQualiter()] more than one address space qualifier "
    													+ "is added to a variable, " + tSym + " in the procedure, " + devProc.getSymbolName() + "\n");
 /*   											DFIterator<Identifier> titer =
    													new DFIterator<Identifier>(rExp, Identifier.class);
