...
/*						if( is1DGangLoop && is1DWorkerLoop ) {
    ...
    protected static void privateTransformation(Procedure cProc, Statement region, String cRegionKind, Expression ifCond,
                                                ...
                                                boolean opt_addSafetyCheckingCode, Set<Symbol> arrayElmtCacheSymbols, boolean isSingleTask, int targetModel, int targetArch ) {
...
/*                SymbolTable targetSymbolTable = AnalysisTools.getIRSymbolScope(privSym, region);
				...
				if( (FirstPrivSymSet != null) && FirstPrivSymSet.contains(privSym) &&
                    ...
                    if( workerPrivOnGlobal ) {
                        ...
                        VariableDeclarator arrayV_declarator =	arrayPrivConv(privSym, localWPSymName, typeSpecs, startList,
                                lengthList, new_proc, region, scope, 1, call_to_new_proc, gwpriv_var.clone(), targetModel);
                        ...
                        if( insertWMalloc && (targetArch != 6) ) {
                        	if( asyncID == null ) {
                        		...
                        		parentRegion.addStatementBefore(region, gpuBytes_stmt);
                        		parentRegion.addStatementBefore(region, malloc_stmt);
                        		if( opt_addSafetyCheckingCode ) {
                        			parentRegion.addStatementBefore(region, gMemAdd_stmt.clone());
                        		}
                        	}
							/*
                            ...
                            if( asyncID == null ) {
                            	...
                            	if( opt_addSafetyCheckingCode  ) {
                            		parentRegion.addStatementAfter(region, gMemSub_stmt.clone());
                            	}
                            	parentRegion.addStatementAfter(region, accTmpFreeStmtstmt);
                            	parentRegion.addStatementAfter(region, gpuBytes_stmt.clone());
                            }
                        }
                    ...
                    if( !isArray && !isPointer ) { //scalar variable
                        ...
                        VariableDeclarator arrayV_declarator =	arrayPrivConv(privSym, localGPSymName, typeSpecs, startList,
                                lengthList, new_proc, region, scope, 0, call_to_new_proc, ggpriv_var.clone(), targetModel);
                        lpriv_var = new Identifier(arrayV_declarator);
                        if( targetModel == 4 ) {
                        	call_to_new_proc.addArgSize(biexp.clone());
                        	call_to_new_proc.addArgTrait(trait_temporary);
                ...
                if( insertMalloc && (targetArch != 6) ) {
                	...
                	if( asyncID == null ) {
                		...
                		parentRegion.addStatementAfter(region, HIFree_stmt);
                		parentRegion.addStatementAfter(region, gpuBytes_stmt.clone());
                    }
                }
                    ...
                    if( !isArray && !isPointer ) { //scalar variable
                        ...
                        if( insertMalloc && (targetArch != 6) ) {
                        	if( asyncID == null ) {
                        		...
                        		parentRegion.addStatementBefore(region, orgGpuBytes_stmt.clone());
                        		parentRegion.addStatementBefore(region, malloc_stmt);
                        		if( opt_addSafetyCheckingCode ) {
                        			parentRegion.addStatementBefore(region, gMemAdd_stmt.clone());
                        		}
                        ...
                        if( insertMalloc && (targetArch != 6) ) {
                            ...
                            if( asyncID == null ) {
                            	...
                            	parentRegion.addStatementAfter(region, tempFree_stmt);
                            	parentRegion.addStatementAfter(region, orgGpuBytes_stmt.clone());
                            }
    ...
    protected static VariableDeclarator arrayPrivConv(Symbol privSym, String symName, List<Specifier> typeSpecs,
                                                      ...
                                                      CompoundStatement scope, int privType, KernelFunctionCall call_to_new_proc, Expression ggpriv_var, int targetArch) {
        // Create an extended array type
        ...
        if( param_sym != null ) {
			...
			/*
			 * if( targetArch != 4 ) { call_to_new_proc.addArgTrait(trait_temporary); }
			 */
        }
		/*
...
*/
    ...
    protected static void reductionTransformation(Procedure cProc, Statement region, String cRegionKind, Expression ifCond,
                                                  ...
                                                  int SIMDWidth, FloatLiteral minCheckValue, int localRedVarConf, int targetModel, int targetArch) {
                ...
                if( isWorkerReduction ) {
                    ...
                    if( workerRedCachingOnShared ) {
                        ...
                        VariableDeclarator arrayV_declarator =	arrayPrivConv(redSym, localWRedSymNameOnGlobal, wTypeSpecs, startList,
                                lengthList, new_proc, region, scope, 1, call_to_new_proc, gwred_var.clone(), targetModel);
                        ...
                        if( insertWMalloc && (targetArch != 6)) {
                            ...
                            if( (asyncID == null) && (confRefStmt != region) ) {
                                postscriptStmts.addStatement(gpuBytes_stmt.clone());
                    ...
                    if( !isArray && !isPointer ) { //scalar variable
                        ...
                        if( targetModel == 4 ) {
                        	arrayV_declarator = arrayPrivConv(redSym, extGRedName, typeSpecs, startList,
                        			lengthList, new_proc, region, scope, 0, call_to_new_proc, extended_var.clone(), targetModel);
                        	call_to_new_proc.addArgSize(biexp2.clone());
                        	call_to_new_proc.addArgTrait(trait_writeonly);
                        } else {
                        	arrayV_declarator = arrayPrivConv(redSym, localGRedSymName, typeSpecs, startList,
                        			lengthList, new_proc, region, scope, 0, call_to_new_proc, ggred_var.clone(), targetModel);
                        	call_to_new_proc.addArgTrait(trait_writeonly);
                        }
	                ...
	                if( isGangReduction ) {
	                    ...
	                    if( !isArray && !isPointer ) { //scalar variable
                        	...
                        	VariableDeclarator pointerV_declarator = scalarSharedConv(redSym, localGRedSymName, typeSpecs,
                        				...
                        				true, false, registerRO, true, preList, postList, targetModel);
                        	call_to_new_proc.addArgTrait(trait_writeonly);
	                        lgred_var = new Identifier(pointerV_declarator);
	                        //PrintTools.println("[reductionTransformation] gang reduction point 3", 2);
	                    } else { //non-scalar variables
