...
int parse_inputs(int argc, char *argv[], unsigned int *chunkSize, unsigned int *lastChunkSize, 
	unsigned int *numTasks, unsigned int *nDevices, unsigned int *asyncMode, unsigned int *inputSize) { 
	int i;
	int error = 0;
	if( argc > 1 ) {
		i = 1;
		while( i<argc ) {
			...
			if(strcmp(argv[i], "-d") == 0) {
				...
				ok = StrToInt(argv[i+1], inputSize);
				if(!ok) {
				...
				printf("    -s S  #set the value of inputSize with S (default: 8)\n");
				printf("    -d D  #set the value of nDevices with D (default: 1)\n");
				printf("    -c C  #set the value of chunkSize with C (default: S^3/D)\n");
				printf("    -am AM  #set the value of asyncMode with AM (default: 2)\n");
				exit(0);
			} else {
