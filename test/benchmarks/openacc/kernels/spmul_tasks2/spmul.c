...
/*
 * This version (version 2) distributes both rowptr and y.
*/
...
#pragma acc kernels loop gang, worker present(x[0:ASIZE], values[0:NZR], colind[0:NZR], rowptr[0:numElements+1], y[0:numElements])
	for( i=0; i<numElements; i++ ) { 
		float tmp = 0.0f;
		for( j=0; j<(rowptr[1+i]-rowptr[i]); j++ ) { 
			tmp = tmp + values[rowptr[i]+j-1]*x[colind[rowptr[i]+j-1]-1];
		}   
		if( tmp != 0.0f ) {
...
int main(int argc, char *argv[]) {
	...
	rowptrList = (int **)malloc(sizeof(int*)*numTasks);	
    ...
    for( i = 0; i < numTasks; i++ ) { 
		...
		rowptrList[i] = (int *)malloc(sizeof(int)*(numElements+1));
    }   
...
*/
	...
	for( i=0; i<=ASIZE; i++ ) {
		int taskID = i / chunkSize;
		j = i % chunkSize;	
		if( taskID >= numTasks ) { 
			taskID = numTasks-1; 
			j = j + chunkSize;
		}
		fscanf(fp10, "%d", &rowptrList[taskID][j]);
		if( (taskID > 0) && (j == 0) ) {
			rowptrList[taskID-1][chunkSize] = rowptrList[taskID][j];
		}
	}
...
LB99:
    ...
    for( i = 0; i < numTasks; i++ ) { 
        ...
        acc_copyin(rowptrList[i], (numElements+1)*sizeof(int));
    }   
...
#pragma acc data copyin(values[0:NZR], colind[0:NZR])
	for( k=0; k<ITER; k++ ) {
#ifdef GRAPH
        ...
        for( i = 0; i < numTasks; i++ ) {
			...
			spmul(colind, rowptrList[i], values, x, y + i*chunkSize, numElements, i, i*chunkSize, asyncID);
		}
#ifdef GRAPH
...
*/
    ...
    for( i = 0; i < numTasks; i++ ) { 
        ...
        acc_delete(rowptrList[i], (numElements+1)*sizeof(int));
    }   
