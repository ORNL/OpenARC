...
*/
		//[DEBUG on Aug. 20, 2021] Changed to dynamic allocation.
		double *u0_r = (double *)malloc(sizeof(double)*NTOTAL);
		double *u0_i = (double *)malloc(sizeof(double)*NTOTAL);
		double *u1_r = (double *)malloc(sizeof(double)*NTOTAL);
		double *u1_i = (double *)malloc(sizeof(double)*NTOTAL);
		double *u2_r = (double *)malloc(sizeof(double)*NTOTAL);
		double *u2_i = (double *)malloc(sizeof(double)*NTOTAL);
		int *indexmap = (int *)malloc(sizeof(int)*NTOTAL);
...
*/
		//[DEBUG on Aug. 20, 2021] Changed to dynamic allocation.
		double *y0_r = (double *)malloc(sizeof(double)*NTOTAL);
		double *y0_i = (double *)malloc(sizeof(double)*NTOTAL);
		double *y1_r = (double *)malloc(sizeof(double)*NTOTAL);
		double *y1_i = (double *)malloc(sizeof(double)*NTOTAL);
...
#pragma acc kernels loop gang worker independent copyout(indexmap[0:NTOTAL])
				for (m = 0; m < NTOTAL; m++)
				{
						i = m % NX;
						k = m / NX;
...
#pragma acc kernels loop gang worker independent
		...
		c_print_results ((char *)"FT", classT, NX, NY, NZ, niter, nthreads,
						total_time, mflops, (char *)"          floating point", verified,
						(char *)NPBVERSION, (char *)COMPILETIME,
						(char *)CS1, (char *)CS2, (char *)CS3, (char *)CS4, (char *)CS5, (char *)CS6, (char *)CS7);
		if (TIMERS_ENABLED == TRUE)
				print_timers ();
