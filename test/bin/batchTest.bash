...
if [ "$OPENARC_ARCH" = "" ]; then
    echo "Environment variable, OPENARC_ARCH, should be set up correctly to run this script; exit."
    exit
fi
	...
	if [ "$TARGET" = "nvl-c" ]; then
		...
		if [ -d "$targetDir" ]; then
			...
			do
				...
				do
					skip_thistest=0
					for skiptest in ${SKIP_LIST[@]}
					do
						if [ "./${skiptest}" = "${example}" ]; then
							skip_thistest=1
							break
						fi
					done
					if [ ${skip_thistest} -eq 1 ]; then
					...
					if [ -f "$targetDir/$example/Makefile" ] && [ -f "$targetDir/$example/O2GBuild.script" ]; then
						echo $example | grep -e "_tuning" -e "openmp4_task" -e "_cash" -e "_manualdeepcopy" -e "_gpu_fpga" > /dev/null
						if [ $? -eq 0 ]; then
							echo "" | tee -a $translog
						...
						echo $example | grep -e "altera" -e "_aspen" -e "_mcl" -e "_cuda" -e "advancedmapping" > /dev/null
						if [ $? -eq 0 ]; then
							echo "" | tee -a $translog
							echo "====> Skip compilation of ${targetDir}/${example}!" | tee -a $translog
							echo "" | tee -a $translog
						...
						do
							if [ $foundMakeCMD -eq 1 ];  then
								...
								foundMakeCMD=0
								#[DEBUG on Dec. 21, 2021] need to enable below on ExCL AMD systems (explorer and cousteau), 
								# but don't know the reason why below statement is needed.
								echo "Found makeCMD : $makeCMD"
							else 
						...
						#[DEBUG on Aug. 6, 2021] Skip execution of fault-injection tests due to possible crashes by the faults.
						echo $example | grep -e "ftinject" > /dev/null
						if [ $? -eq 0 ]; then
							echo "" | tee -a $runlog
							echo "==> Target: ${targetDir}/${example}" | tee -a $runlog
							echo "    Skip execution of ${targetDir}/${example}!" | tee -a $runlog
							echo "" | tee -a $runlog
							continue
						fi
